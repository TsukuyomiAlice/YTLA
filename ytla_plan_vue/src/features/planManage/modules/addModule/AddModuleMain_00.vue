<template>
  <h1>{{ plan?.name }} - {{ $t('planManage.AddModuleMain_00_000')}}</h1>
  ...
  <div v-if="currentView === 'type'" class="module-category">
    <div v-for="(modules, type) in filteredByType" :key="type">
      <h2>{{ $t(`${type}.type_name`) }}</h2>
      <h2>{{ $t(`${type}.type_description`) }}</h2>
      <div v-for="module in modules" :key="module.moduleSubType" class="module-item" @click="handleModuleClick(module)">
        <h3>{{ $t(`${module.moduleType}.${module.moduleSubType}_subtype_name`) }}</h3>
        <h3>{{ $t(`planManage.AddModuleMain_concept_${module.moduleConcept}`) }}</h3>
        <p>{{ $t(`${module.moduleType}.${module.moduleSubType}_subtype_description`) }}</p>
      </div>
    </div>
  </div>
  ...
  <div v-else class="module-category">
    <div v-for="(modules, concept) in filteredByConcept" :key="concept">
      <h2>{{ $t(`planManage.AddModuleMain_concept_${concept}`) }}</h2>
      <div v-for="module in modules" :key="module.moduleSubType" class="module-item" @click="handleModuleClick(module)">
        <h3>{{ $t(`${module.moduleType}.${module.moduleSubType}_subtype_name`) }}</h3>
        <h3>{{ $t(`${module.moduleType}.type_name`) }}</h3>
        <p>{{ $t(`${module.moduleType}.${module.moduleSubType}_subtype_description`) }}</p>
      </div>
    </div>
  </div>
...
import ReturnToPlanDashboardButton from '@/features/planManage/components/ui/ReturnToPlanDashboardButton.vue'
import SwitchToModuleConceptButton from '@/features/planManage/components/ui/SwitchToModuleConceptButton.vue'
import SwitchToModuleTypeButton from '@/features/planManage/components/ui/SwitchToModuleTypeButton.vue'
...
import { getRegisteredModules, getModuleConfig } from '@/core/modules/_type/registries/moduleRegistry.ts'
