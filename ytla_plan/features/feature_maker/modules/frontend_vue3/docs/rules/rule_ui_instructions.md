...
对于UI组件的Props和Emits定义，优先采用以下方式：

在非必要的情况下，直接在Vue文件中使用defineProps保留定义，只有在必须时才生成对应的definitions文件。
如果已经生成了定义文件，除非用户明确要求，一般保留现状

#### 推荐方式（优先使用）

对于大多数UI组件，推荐直接在Vue文件中使用defineProps定义类型：
...
#### 需要生成definitions文件的情况

仅在以下情况下，才考虑在definitions目录下创建\[组件名]Type.ts文件：

1. **作为主组件/布局组件**：该UI组件是所在目录的主组件(components)或布局(layouts)，需要被其他模块广泛引用
2. **类型需要复用**：Props和Emits类型需要被多个组件或composables复用
3. **类型定义复杂**：Props和Emits类型定义非常复杂，单独维护更清晰

#### 作为子组件的情况

作为子组件时：
- 优先直接在Vue文件中使用defineProps定义类型
- 可以继承或复用父组件的类型定义（如有需要）
...
1. **关注点分离**：Vue文件仅负责视图，逻辑必须移动到TS文件；composables文件中不应包含类型定义内容，类型定义应正确地移到definitions目录
2. **职责清晰**：
   - `.vue`文件：仅包含HTML模板、导入语句、样式导入
   - `.ts`文件（composables）：包含所有业务逻辑、响应式数据、计算属性、方法，**不应包含类型定义**
   - `.ts`文件（definitions）：包含Props、Emits等类型定义
...
1. **明确重构目标**：确定重构的具体原因和目标
   ...
   - **类型定义分离：将composables文件中的类型定义移动到definitions目录**（重要！）
   - 性能优化：提升组件性能
