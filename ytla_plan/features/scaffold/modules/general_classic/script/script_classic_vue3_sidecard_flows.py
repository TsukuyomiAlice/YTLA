...
def create_flows_file(base_path, module_type, module_sub_type):
    """
    生成 SideCard 具体功能实现目录的 flows 文件（可选）
    ...
    file_path = Path(base_path) / module_type / "cards" / module_sub_type / "flows" / f"{module_sub_type}FlowManager.ts"
...
{module_sub_type}ModuleFlowManager.registerFlow('{module_sub_type}-main-steps', [
  defineAsyncComponent(() => import('@/features/{module_type}/cards/{module_sub_type}/components/{module_sub_type_upper}Card.vue')),
])
'''
