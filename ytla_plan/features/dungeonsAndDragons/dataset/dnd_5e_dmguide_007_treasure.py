...
本章详述了魔法物品和冒险中的藏宝地、魔法物品和普通物品，以及除此之外的其他奖励方案宝藏类型 Types of Treasure宝藏有许多种形式。
...
随机宝藏 Random Treasure你可以使用以下数页中附带的表格随机生成若干宝藏，以让怪物携带、巢穴贮藏或其他地方藏匿。宝藏所在位置由你决定。关键在于要确保玩家们能够感觉到自己以游戏中的角色战胜危险并获得奖励。
宝藏表 Treasure Tables你可以根据怪物的挑战等级来随机分配宝藏。表格分为挑战等级 0 至 4，挑战等级 5 至 10，挑战等级 11 至 16 和挑战等级 17 或更高。你还可以使用这些随机表格来确定某个怪物携带了多少钱（相当于 D&D 中的零花钱）或在某个较大的宝库中发现的财富总金额。
使用个体宝藏表 Using the Individual TreasureTables一张“个体宝藏”表可以帮你随机选定单个生物自身携带的宝藏。你可以在处理一些不喜欢收藏财宝的怪物时使用这些表格来确定怪物的被害人意外遗留下的宝藏。
...
使用库藏宝藏表 Using the Treasure Hoard Tables一张“库藏宝藏”表帮你随机选定的宝藏可应用于藏品量大的宝藏，一大群生物共同积累的财富（比如一个兽人部族或大地精军队），一个喜强大生物因个人兴趣而聚敛的宝库（比如一条龙），或者在队伍完成一项任务后作为报答者的馈赠奖励。你也可以将这些宝藏分开存放，以让冒险者们分几次寻获这些财宝。
...
宝石 Gemstones如果一个宝库的藏品包含宝石，则你可以使用下列表格并按其价值来随机选定被找到的是宝石种类。你可以设定其内只有一种宝石然后只骰一次，或者多次掷骰来创造一个包含多种宝石的宝石堆。
艺术品 Art Objects如果一个宝库的藏品包含艺术品，则你可以使用下列表格并按其价值来随机选定被找到的艺术品。你可以多次掷骰来确定宝库中的艺术品藏品。而同个宝库里也会存在多件相同的艺术品。
魔法物品 Magic Items魔法物品通常从被打败的怪物身上，以及发掘出的失落地窖里搜刮而来。这些物品能够为角色们发挥一些难以替代的作用，又或者以奇异的方式来辅助角色本身的能力。
稀有度 Rarity每件魔法物品都有一个稀有度：普通、非普通、珍稀，极珍稀或传说。常见的魔法物品（比如治疗药水 potion of healing）供给量最充足。而一些传说物品（比如夸力许装置 apparatusof Kwalish）往往都是独一无二的物品。游戏中设定了那些最强大的物品在数个世纪前秘密制成，随后却因战争、灾难和意外而逐渐遗失下落。因而就算非普通级的物品也难以轻易制成，而许多魔法物品则作为古董被完好的保存了下来。
...
购买与出售 Buying and Selling大多数魔法物品极珍稀而无法买到，但你仍可以在你战役中自行设定其买卖状况。普通的物品（比如一瓶治疗药水potion of healing）可以从炼金师、草药师或者施法者手中购买，但很少会以商店购物的简单形式实现，此时卖方或许会要求为其提供一项服务而不是以钱币来进行交易。
...
鉴定魔法物品 Identifying a Magic Item一些魔法物品与某些非魔法物品非常相似而难以鉴别，而另一些魔法物品则毫不掩饰的散发着自己的魔法性质。无论魔法物品的外观如何，处理这些物品的角色都会感觉到其本身的非凡之处。不过你还是必须进行某些处理才能获知一件魔法物品的属性。
...
变体：更难鉴定 More Difficult Identification如果你喜欢魔法物品更有神秘感，则可以考虑排除掉短休鉴别魔法物品属性的设定，并要求必须通过法术鉴定术identify 或测试，或者两者兼备才可解释魔法物品的属性。
同调 Attunement一些魔法物品需要一个生物与自己形成某种联结后才能使用其魔法属性。这种称为同调 attunement 的联结通常还有某些先决条件。如果同调某件魔法物品的先决条件是某个职业，则尝试与该物品同调的生物必须是该职业才能与之进行同调。如果该职业限制为施法职业，而拥有法术位且使用相应职业法术列表的怪物也具备与之同调的资格。如果先决条件为施法者，而生物某特质或特性令其可施展至少一项法术（不能利用魔法物品或相似状况），则该生物也具备同调资格。
...
被诅咒物品 Cursed Items某些被诅咒的魔法物品会困扰其使用者，有时甚至使用者不再使用该物品也会在很长时间里受其影响。魔法物品的描述中会详细说明该物品是否被诅咒。虽然一些逸闻学识中会提及这些诅咒，但大多数鉴定物品的方法（包括法术鉴定术identify）都无法揭示这些诅咒。而这些诅咒的效应显露时，往往都会让其使用者感到十分意外。
与一件被诅咒物品同调后其同调联结在诅咒被破除前无法被终止，破除诅咒的方式之一便是对其施展法术移除诅咒remove curse。
魔法物品类别 Magic Item Categories每件魔法物品都属于一个类别：护甲、魔药、戒指、权杖、卷轴、法杖、魔杖、武器或奇物。
护甲 Armor除非一件护甲的描述另有说明，否则护甲必须着装才能让其魔法发挥作用。
一些魔法护甲会指定其护甲类型，例如链甲或板甲。如果一件魔法护甲没有指定其护甲类型，则你可以随意或随机为其指定一个类型。
魔药 Potions不同种类的魔法液体被归类为几个种类的魔药：由魔法草药调制的药剂，来自魔泉或圣泉的水，以及用于生物或物件的油。大多数魔药都盛装的一盎司液体。
魔药是可消耗的魔法物品。喝药水或给其他角色服用药剂需要花费一个动作，而应用油类可能需要更长时间（具体见相应描述）。魔药使用过后即被耗尽且立即生效。
戒指 Rings魔法戒指为那些有幸找到它们的人提供神奇的魔力。除非戒指的描述另有说明，否则戒指必须着装在手指或类似的肢体结构上，才能发挥其魔法作用。
权杖 Rods一根魔法权杖可以是一支节杖或一支沉重的圆棒，其实体通常由金属、木头或骨头制成。其外形大约 2 到 3 尺长，1寸厚，2 到 5 磅重。
卷轴 Scrolls大多数卷轴都是以文书形式保存的法术，而少数的卷轴所蕴含的独特咒语还可以生成某些强效的结界。无论其内容是什么，卷轴的实体都是一卷纸，有时会附在木棒上，通常都保存在由象牙、翡翠、皮革、金属或木材等材料制作的管筒中。
...
法杖 Staffs一根法杖大约 5 到 6 尺长。法杖的外形千差万别：有的直径均匀而光滑，有的则扭曲变形，有的由木头制成，有的则由抛光的金属或水晶组装而成。根据材质的不同，一根法杖的重量介于 2 到 7 磅之间。
除非法杖的描述另有说明，否则法杖都可以被用作一根长棍 quarterstaff。
魔杖 Wands一根魔杖大约长 15 寸，由金属、骨头或木头制成。其末端由金属、水晶、石头或其他一些材料制成。
武器 Weapons不管是因某些堕落目标制成，还是为最崇高的骑士精神而铸造，所有的魔法武器都是无数冒险者们趋之若鹜的宝贝。
...
奇物 Wondrous Items奇物包括一些着装物品如靴子 boots、腰带 belts、披肩capes、手套 gloves，以及各种珠宝 jewelry 和装饰品如护身符 amulets、胸针 brooches 和饰环 circlets 等。袋子 bags、地毯 carpets、水晶球 crystal balls、塑像 figurines、号角 horns、乐器 musical instruments 和其他物件也属于这一类别。
变体：混合药水 Mixing Potions角色可能会在正处于某个药水的效应期间喝下另一种药水，或者会将几种药水倒进同一个容器中。而那些用来制造药水的奇异成分可能会导致不可预知的相互作用。
当角色把两种药水混合在一起时，你可以按表格“可混合药水”所示进行掷骰。如果两个以上的组合混一起，则为随后每份加入的药水再进行一次掷骰并将结果结合。除非其效应会立即显现，否则你可以在其需要显现效应时才揭晓这些结果。
着装与持用物品 Wearing and WieldingItems一些魔法物品需要着装或持用才能使用其属性。一件需要着装的魔法物品必须按照预定的方式穿戴：靴子必须穿在脚上，手套必须戴在手上，帽子和头盔必须戴在头上，戒指必须戴在手指。魔法护甲必须着装完备，盾牌必须在手臂上固定妥当，斗篷必须系在肩上。武器则必须拿在手中。
...
复数同类物品 Multiple Items of the Same Kind用常识来判断是否可以着装多于一件的同类魔法物品。
...
变体：阅卷失误 Scroll Mishaps一个尝试用法术卷轴 spells croll 施展法术的生物，必须进行一次 DC 10 的智力豁免。如果豁免失败，则按表格“阅卷失误”进行掷骰。
成对物品 Paired Items成 对 的 物 品 （ 比 如 靴 子 boots 、 护 腕 bracers 、 护 手gauntlets 和手套 gloves）只有当它们的两件都被着装时才能让相应的增益生效。例如，一名单脚穿着跳跑之靴 boot ofstriding and springing，另一脚却踏着一只精灵之靴 bootofelvenkind 的角色无法获得这些靴子提供的任何增益。
激活物品 Activating an Item一些魔法物品需要其使用者做一些特别的事情才能激活，比如拿着物品说出相应的命令语。每个物品品类或单个物品的描述中都详述了激活该物品的方法。而某些特定的物品会使用下列其一或多项作为其激活规则。
如果某物品需要用一个动作激活，则该动作本身并不作为“使用物品 Use an Item”动作，因此一些特性，如游荡者的“快手 Fast Hands”特性就无法用于激活这些物品。
命令语 Command Word命令语以说话方式发出的一个单词或短句，用以驱使物品开始工作。需要命令语的魔法物品无法在声音被压制的区域里激活，比如法术沉默术 silence 的效应范围里。
消耗品 Consumables一些物品被激活后将被消耗殆尽。药水或灵药以服用的方式，而油类则用在相应的实体上。被阅读使用后的卷轴其上的文字消失不见。使用过后，消耗品类的物品也随之失去其魔法。
魔法物品图纸 Magic Item Formulas一份魔法物品的图纸解释了如何制作某一特定的魔法物品。
...
法术 Spells一些魔法物品可以让其使用者从物品中施展法术。这些法师以法术的最低环阶施展，其不消耗使用者的法术位，也不需要任何构材，除非相应物品的描述中另有说明。这些法术使用它正常的施法时间、施法距离和持续时间，如果法术需要专注则该物品的使用者还必须对物品进行专注。许多物品（比如药水），可以直接跳过施法而直接赋予法术的效应并维持通常的持续时间。而一些特定的物品与该规则不同，会改变法术的施法时间，持续时间以及法术的其他内容。
...
充能 Charges一些魔法物品需要花费充能来激活属性。一件物品保留的充能次数可以通过对其施展鉴定术 identify 来查明，与之同调的生物也可以通过直觉得知。此外，当一件物品恢复充能时，与之同调的生物也会在第一时间得知其拥有的充能总量。
魔法物品自愈 Magic Item Resilience大多数魔法物品都是非凡工艺的作品。精心的制作结合魔法强化，使这些魔法物品的耐用性绝不逊色于相似的非魔法物品。除药水与卷轴外的大多数魔法物品都对所有类型伤害具有抗性。神器则尤其坚不可摧，而只能通过一些极端手段才能破坏。
专有特征 Special Features你可以思考一件魔法物品的幕后故事来为其增加独特性。
...
变体：无法充能的魔杖 Wands That Don’t Recharge一根典型的魔杖拥有可消耗的充能次数。如果你想让魔杖成为一种有限的资源，则你可以设定某些魔杖彻底无法充能。你可以考虑为这种魔杖设置一个保底充能次数，其最高值为 25。这些充能一旦被消耗便永远无法恢复。
随机魔法物品 Random Magic Items当你使用一个“库藏宝藏”表来随机选定宝库的内容物，且你的骰值示意你决定其内的魔法物品时，你可以使用这里附带的诸表格并掷骰来选定指定的魔法物品。
魔法物品 A~Z魔法物品以物品名字母顺序排列。每个魔法物品的描述都记述了物品的名字、类型、稀有度，以及它的魔法属性。
精金护甲 Adamantine Armor护甲（中型或重型，非兽皮甲），非普通这套护甲以一种现存最坚硬的物质——精金 adamantine强化。着装该护甲时，任何命中你的重击都会改为普通命中。
炼金壶 Alchemy Jug奇物，非普通这种陶制壶看起来可以承装 1 加仑的液体，但无论壶中是空还是满，其重量都是 12 磅。即使壶内空无一物，摇动它时也依然会发出液体晃动的声音。
你可以用一个动作命令此壶产生下表列出液体中的一种。
然后，你可以用一个动作打开壶，并以 2 加仑/分钟速度倒出液体。壶中产生液体的体积最大值取决于你指定的液体类型。
从此壶开始生成液体到次日黎明前，它将无法再生成超过其上限的液体或是产生另一种液体。
弹药 Ammunition，+1、+2 或+3武器（任何弹药），非普通（+1），珍稀（+2），极珍稀（+3）你使用此魔法弹药发动攻击检定和伤害掷骰时获得加值。
加值的数值取决于弹药的稀有度。此弹药一旦击中目标即不再具有魔法效果。
健康护符 Amulet of Health奇物，珍稀（需同调）着装此护符时，你的体质值变为 19。如果你的体质值在未着装护符时已经大于或等于 19，则此护符无效。
反侦护符 Amulet of Proof Against Detection andLocation奇物，非普通（需同调）着装此护符时，你不会被侦测类魔法发现。你无法被指定为这类魔法的目标，魔法制造的探测器也无法觉察到你。
位面护符 Amulet of The Planes奇物，极珍稀（需同调）着装此护符时，你可以用一个动作来点名一处你熟悉的另一存在位面地点，然后进行一次 DC 15 的智力检定。如果检定通过，则你施展出法术异界传送 plane shift。如果检定失败，你将和 15 尺内的每个生物和物件一起被传送到一个随机地点。骰一次 d100。如果骰值为 1~60，则你被传送到你点名位面中的随机地点。如果骰值为 61~100，则你被随机传送到一个存在位面。
活化盾 Animated Shield护甲（盾牌），极珍稀（需同调）你可以持握此盾时并使用附赠动作说出其命令语使其活化。盾牌将飞到空中并悬浮在你所占据空间中保护你，如同你正持用它一般，且同时解放了你的双手。
盾牌保持活化 1 分钟，或直至你陷入失能、死亡或使用一个附赠动作终止其效应。如果这时你有一只手空着，盾牌将落回你手里。否则它会掉落在地上。
夸力许装置 Apparatus of Kwalish奇物，传说这个物品乍一看是个重达 500 磅的密封大铁桶。成功通过一次 DC 20 的智力（调查）检定后可以发现铁桶上有一个隐藏的把手。扳动把手将打开一扇舱门，其大小可以让两名中型或更小体型的生物爬入。舱体尽头是十个拉杆。通常拉杆的位置都在中间，且可以向上推或向下拉。特定的拉杆被拉动时，装置将变形成类似巨大龙虾的外形。
夸力许装置是一个大型物件，其资料如下：
...
护甲 Armor，+1、+2 或+3护甲（轻型、中型或重型），珍稀（+1），极珍稀（+2），传说（+3）你着装该护甲时 AC 获得加值。加值的数值取决于护甲的稀有度。
坚不可摧护甲 Armor of Invulnerability护甲（板甲），传说（需同调）着装此护甲时，你对非魔法伤害拥有抗性。此外，你可以用一个动作让自己免疫非魔法伤害，持续 10 分钟或直到你不再着装该此护甲为止。使用后，直到次日黎明前你都无法再次启动该专有动作。
抗性护甲 Armor of Resistance护甲（轻型、中型或重型），珍稀（需同调）着装此护甲时，你对一种伤害类型具有抗性。该伤害类型由 DM 决定或使用下表随机选定。
易伤护甲 Armor of Vulnerability护甲（板甲），珍稀（需同调）着装此护甲时，你对钝击、穿刺或挥砍中的一种伤害类型具有抗性。该伤害类型由 DM 决定或随机选定。
...
吸矢盾 Arrow-Catching Shield护甲（盾牌），珍稀（需同调）持用此盾牌时，你对抗远程攻击的 AC 获得 +2 加值。该加值将加在盾牌通常提供的 AC 加值上。此外，当攻击者对你身边 5 尺内的目标发动远程攻击时，你可以使用反应以替代该目标成为这次攻击的目标。
屠杀箭 Arrow of Slaying武器（箭），极珍稀屠杀箭是专门用于杀死某类生物的魔法武器。不同屠杀箭针对的范围可能不同。例如，可能同时存在一种龙类屠杀箭arrows of dragon slaying 以及一种蓝龙屠杀箭 arrows of bluedragon slaying。属于屠杀箭关联的生物类型、种族或团体的生物受到这种箭伤害时，该生物必须进行一次 DC 17 的体质豁免。豁免失败的生物将额外受到 6d10 的穿刺伤害，豁免成功则减半。
屠杀箭一旦对某生物造成了额外伤害，它就会立即变成非魔法的箭。
除箭之外也存在一些拥有类似效应的其他类型弹药，例如弩使用的屠杀矢，但箭是最常见的。
魔豆之袋 Bag of Beans奇物，珍稀这个沉重的布袋里装着 3d4 颗干燥的豆子。布袋重 1/2磅，外加每颗豆子重 1/4 磅。
...
d100 效应01 长出 5d4 个毒蕈。吃掉毒蕈的生物必须掷一个任意面数的骰子。如果骰值为奇数，则食用者必须进行一次DC 15 的体质豁免，豁免失败则受 5d6 的毒素伤害并中毒 1 小时。如果骰值为偶数，则食用者获得 5d6 的临时生命值，效应持续 1 小时。
...
吞噬袋 Bag of Devouring奇物，极珍稀这个表面上看起来像是次元袋 bag of holding 的袋子实际上是某个巨大异次元生物的进食口。将袋子内外翻转就可以关闭进食口的通道。
...
次元袋 Bag of Holding奇物，非普通这个袋子内部的实际空间比外表看起来要大。它深约 4尺，袋口直径约 2 尺，可以容纳至多 500 磅，体积不超过 64立方尺的东西。无论装了什么，这个袋子总是重 15 磅。从袋子里取出东西需要花费一个动作。
...
魔术袋 Bag of Tricks奇物，非普通这种普通布袋有三种不同颜色的版本：灰色、铁锈色和棕褐色。布袋看起来是空无一物，但把手伸进去会摸到一个小小的毛绒物件。整个袋子重 1/2 磅。
...
力场珠 Bead of Force奇物，珍稀这个小型黑色球体直径 3/4 寸，重 1 盎司。通常都会有1d4+4 颗珠子保存在一起。
...
矮人腰带 Belt of Dwarvenkind奇物，珍稀（需同调）着装此腰带时，你获得以下增益：
• 你的体质增加 2，但不能超过 20。
...
• 你获得 60 尺黑暗视觉。
• 你可以说、读、写矮人语。
巨人之力腰带 Belt of Giant Strength奇物，多种稀有度（需同调）着装此腰带时，你的力量值变为腰带赋予的数值。如果你未着装该腰带时，力量值已经大于或等于腰带赋予的数值，则该物品无效。
这类腰带共有六种，稀有度各有不同，分别对应六种巨人。石巨人之力腰带 Belt of stone giant strength 和霜巨人之力腰带 Belt of frost giant strength 外形不同但效果一样。
狂战斧 Berserker Axe武器（任何斧），珍稀（需同调）你使用此魔法武器进行攻击检定和伤害掷骰时获得+1 加值。此外，与该武器同调后，你的生命值上限将增加等于你等级的数值。
...
精灵之靴 Boots of Elvenkind奇物，非普通着装这双靴子时，无论在何种表面上移动，你都不会发出脚步声。你进行有关无声移动的敏捷（隐匿）检定时具有优势。
浮空之靴 Boots of Levitation奇物，珍稀（需同调）着装这双靴子时，你可以用一个动作对自身施展法术浮空术 levitate，次数不限。
速度之靴 Boots of Speed奇物，珍稀（需同调）着装这双靴子时，你可以获得一个互击脚后跟的附赠动作项。执行该附赠动作后，你的移动速度将加倍，且任何生物对你发动借机攻击时都承受劣势。再次互击脚后跟将终止该效应。
靴子的属性持续使用 10 分钟后，其魔法效应将随之消失直至你完成一次长休才能再次使用。
跳跑之靴 Boots of Striding and Springing奇物，非普通（需同调）着装这双靴子时，你的步行速度变为 30 尺，除非你原本的步行速度更快。你的速度此时不会因为受阻或者穿着护甲而减慢。此外，你跳跃的距离是通常的三倍，但不能超过你剩余的移动距离。
雪地之靴 Boots of the Winterlands奇物，非普通（需同调）这双加绒靴子温暖且舒适。着装这双靴子时，你获得以下增益：
• 你对寒冷伤害具有抗性。
• 你忽略冰或雪造成的困难地形对移动的影响。
• 你能够在没有任何其他保护的情况下忍受低至 -50 华氏度的气温。如果你穿着厚重的衣服，你能够忍受低至 -100 华氏度的气温。
命令元素水钵 Bowl of Commanding WaterElementals奇物，珍稀这个水钵装满水时，你可以用一个动作并说出水钵的命令语以召唤一个水元素 water elemental，其效应相当于你施展出法术召唤元素生物 conjure elemental。直到次日黎明前，该水钵都无法再次以该方式使用。
水钵直径约 1 尺，深约半尺。它的重量为 3 磅，能装承大约 3 加仑水。
射手护腕 Bracers of Archery奇物，非普通（需同调）着装这对护腕时，你获得长弓 longbow 和短弓 shortbow的熟练项。你使用这些武器发动远程攻击时，其伤害掷骰获得+2 加值。
防御护腕 Bracers of Defense奇物，珍稀（需同调）着装这对护腕时，如果你没有穿着护甲也没有使用盾牌，则你的 AC 获得+2 加值。
命令元素火盆 Brazier of Commanding FireElementals奇物，珍稀当火焰在这个铜盆中燃烧时，你可以用一个动作说出火盆的命令语并以此召唤一个火元素 fire elemental，其效应相当于你施展出法术召唤元素生物 conjure elemental。直到次日黎明前，火盆都无法再次以该方式使用。
火盆的重量为 5 磅。
护盾胸针 Brooch of Shielding奇物，非普通（需同调）着装此胸针时，你获得对力场伤害的抗性，并免疫法术魔法飞弹 magic missile 的伤害。
飞天扫帚 Broom of Flying奇物，非普通这把扫帚平时只是一把 3 磅重的普通木制扫帚。如果你骑在它上面并说出它的命令语，它就会悬浮在你身下且可以载着你在空中飞行。它的飞行速度为 50 尺。虽然飞天扫帚最多可以搭载 400 磅的重物，但 200 磅以上的载重会将它的飞行速度降低到 30 尺。扫帚在你着陆之后便会停止悬浮。
你可以说出命令语并指明 1 里内一个你熟悉的地点，以此命令扫帚自行飞到该处。如果扫帚在你周围 1 里以内，则你可以用另一个命令语让它回到你身边。
祈神蜡烛 Candle of Invocation奇物，极珍稀（需同调）每支祈神蜡烛都为一位选定的神祇定制专用，且具有与神祇相同的阵营。蜡烛的阵营可以用侦测善恶 detect evil andgood 辨别。蜡烛对应的神祇或阵营由 DM 选择或随机决定。
...
水下呼吸帽 Cap of Water Breathing奇物，非普通在水下着装此帽时，你可以用一个动作说出其命令语以在你头部周围创造出一个供你呼吸的空气泡。空气泡持续存在，直至你再次说出命令语、摘掉帽子或是离开水下。
郎中披肩 Cape of the Mountebank奇物，珍稀这件披肩闻起来有一股淡淡的硫磺味。着装该披肩后，你可以用一个动作施展法术任意门 dimension door。使用该属性后直至次日黎明前都无法再次以该方式使用该物品。
以此消失时，你将在原本的位置留下一团烟雾，之后你伴随着一团类似的烟雾出现在目的地。烟雾会在你离开和到达的位置造成轻度遮蔽，并在下一个你的回合结束时消散。一阵微风或更强的风势可以将这种烟雾吹散。
飞毯 Carpet of Flying奇物，极珍稀你可以用一个动作说出让飞毯起飞或悬停的命令语。只要你在飞毯周围 30 尺内，它就会按照你的口令移动。
飞毯有四种不同的尺寸，其具体如何则由 DM 进行选择或随机决定。
飞毯的载重上限为表中数值的两倍，但超过正常载重时飞毯的飞行速度减半。
命令元素香炉 Censer of Controlling Air Elementals奇物，珍稀当熏香在这个香炉中燃烧时，你可以用一个动作说出香炉的命令语并以此召唤出一个气元素 air elemental，其效应相当于你施展出法术召唤元素生物 conjure elemental。直到次日黎明前，香炉都无法再次以该方式使用。
香炉宽 6 尺，高 1 尺，重 1 磅。其外形类似于一个带有精美装饰杯盖的酒杯。
开门钟琴 Chime of Opening奇物，珍稀这种空心金属管长约 1 尺，重 1 磅。你可以用一个动作敲击它，在将它指向 120 尺内一个能被打开的物件，比如门、盖子或锁。被敲击的钟琴会发出一个清晰的音调。只要声音可以传到目标物件处，该物件上面的一个锁或者闩就会被打开。
如果物件上没有锁闭的锁或闩，则物件本身被打开。
开门钟琴可使用 10 次。第 10 次后它将碎裂而失去用处。
爆裂头饰 Circlet of Blasting奇物，非普通着装此头饰后，你可以用一个动作施展出法术灼热射线scorching ray。使用此法术攻击的攻击加值为 +5。直到次日黎明前，头饰都无法再次以该方式使用。
节肢斗篷 Cloak of Arachnida奇物，极珍稀（需同调）这件精美的服饰由黑色丝绸制成，其上用银色丝线绘成网状花纹。着装此斗篷后，你获得以下增益：
• 你获得对毒素伤害的抗性。
• 你获得和步行速度相同的攀爬速度。
...
移位斗篷 Cloak of Displacement奇物，珍稀（需同调）着装此斗篷后，它将投射出你的幻影，使你看起来像是站在一个和你实际位置稍有偏差的地方。任何生物对你发动攻击时都将承受劣势。你受到伤害时斗篷会暂时失效，直至你的下一回合开始恢复效应。当你陷入失能、束缚状态或因其他原因而无法行动时，斗篷的效应也随之被压制。
精灵斗篷 Cloak of Elvenkind奇物，非普通（需同调）着装此斗篷并拉起兜帽后，它将变换颜色为你提供伪装。
任何试图看到你的感知（觉察）检定都将承受劣势，且你在使用敏捷（隐匿）检定进行躲藏时具有优势。褪下或拉起兜帽需要一个动作。
蝙蝠斗篷 Cloak of the Bat奇物，珍稀（需同调）着装此斗篷后，你的敏捷（隐匿）检定具有优势。在微光光照或黑暗环境中，你可以用双手抓住斗篷边沿，利用它以40 尺的速度飞行。如果你没能抓住斗篷边沿或不再处于微光光照或黑暗环境中，则你失去此飞行速度。
着装此斗篷且身处微光光照或黑暗环境时，你可以用一个动作对自己施展出法术变形术 polymorph，并以此变成一只蝙蝠 bat。你在变成蝙蝠时保留自身的智力、感知和魅力属性。
直到次日黎明前，斗篷都无法再以该方式使用。
蝠鲼斗篷 Cloak of the Manta Ray奇物，非普通着装此斗篷并拉起兜帽后，你获得 60 尺游泳速度，并能在水下呼吸。褪下或拉起兜帽需要一个动作。
隐身斗篷 Cloak of Invisibility奇物，传说（需同调）着装此斗篷后，你可以把兜帽拉到头上以使自己隐形。你身上着装和携带的所有东西也会随你隐形。褪下兜帽会使你显形。褪下或拉起兜帽需要一个动作。
隐身斗篷的效应至多能持续 2 小时，持续时间以 1 分钟为单位扣除。使用 2 小时后，斗篷将暂时失效。停止使用斗篷达 12 小时可以为它恢复 1 小时的使用时间。
防护斗篷 Cloak of Protection奇物，非普通（需同调）着装此斗篷后，你的 AC 和豁免获得+1 加值。
水晶球 Crystal Ball奇物，极珍稀或传说（需同调）水晶球通常直径 6 寸，是一种非常稀有的魔法物品。接触水晶球时，你可以用它施展出法术探知 scrying（豁免 DC17）。
...
力场魔方 Cube of Force奇物，珍稀（需同调）这个立方体棱长约 1 寸，每个面上都有一个可以按压的独特标记。它共有 36 发充能，每天黎明时会恢复 1d20 发已消耗的充能次数。
...
异界之门魔方 Cubic Gate奇物，传说这个棱长 3 寸的立方体散发着明显的魔法能量。立方体的六个面分别对应六个位面，其中之一是物质位面其他则由DM 决定。
...
无尽水瓶 Decanter of Endless Water奇物，非普通这个 2 磅重，带瓶塞的瓶子在摇晃时会发出液体晃动的声音，仿佛其中有水一般。
...
幻象牌组 Deck of Illusions奇物，非普通这个匣子里装着一套羊皮纸制的卡牌。一套完整的牌组有 34 张卡，但作为宝藏获得牌组时通常会缺少 1d20-1 张卡。
...
万象无常牌 Deck of Many Things奇物，传说这种由象牙或羊皮纸制成的卡牌通常存放在盒子或袋子里。一套牌大多数时候（75%）只有 13 张卡，但有时也会有22 张。
...
只有法术 许愿术 wish 或神灵干涉程度的力量才能终止该非玩家角色对你的敌意。
废墟 Ruin。你失去魔法物品以外一切你持有或拥有的财产。可动产将直接消失；在最低限度修改现实的前提下，你还将失去你拥有的所有生意、建筑物和土地。任何证明你拥有任何财产的文件也将因此卡而消失。
头骨 Skull。你召唤出一名死亡化身 avatar of death——身穿破烂黑袍，手持幽灵镰刀，幽魂般的一个类人生物骷髅。
它将在你周围 10 尺内一处 DM 指定的位置出现并对你发动攻击，同时警告其他人你必须要独自面对这场战斗。化身在将你杀死或自身生命值降至 0 后消失。如果有人试图在这场战斗中帮你，则该人物将因此召唤出自己的死亡化身。被死亡化身杀死的生物无法被复活。
...
关于树敌 A Question of Enmity万象无常牌中有两张卡牌可能会使某个存在对抽卡的角色产生敌意。“火焰”卡牌导致的敌意是显而易见的，角色会因此多次遭遇魔鬼的恶毒算计。但找到这名魔鬼却并不容易。在最终对抗魔鬼之前，角色可能会多次与它的盟友和手下发生冲突。
“浪人”卡牌导致的敌意来源于抽卡的角色以为是朋友或盟友的角色，因此不易察觉。作为 DM，你应该等待最有戏剧效果的时刻展示这种敌意，让玩家们不断猜测背叛者是谁。
防御者 Defender武器（任意剑），传说（需同调）使用此魔法武器发动的攻击检定和伤害掷骰获得+3 加值。
...
恶魔护甲 Demon Armor护甲（板甲），极珍稀（需同调）着装该护甲后你的 AC 获得+2 加值，且可以理解和说出深渊语。另外，护甲附带利爪般的铁护手可以令你用手进行徒手打击时视为魔法武器并造成挥砍伤害，你以此发动的攻击检定和伤害掷骰获得+1 加值。
诅咒 Curse。着装这件被诅咒的护甲后，除非你被施予法术移除诅咒 remove curse 或类似的魔法效应，否则你无法将其卸下。着装该护甲后，你对恶魔进行的攻击检定，以及对抗恶魔法术和特殊能力时进行的豁免检定均具有劣势。
次元镣铐 Dimensional Shackles奇物，珍稀你可以用一个动作来以此镣铐束缚一个失能的生物。镣铐可以自行调整适应从小型到大型体型的生物。除普通镣铐的作用之外，次元镣铐还能阻止被其束缚生物进行异次元移动，包括传送和异位面旅行。但是，镣铐不会阻止生物穿过一道连接异位面的传送门。
你和你指定的任何生物能用一个动作解除此镣铐。每隔30 日，被束缚的生物可以进行一次 DC 30 的力量（运动）检定，成功则破坏镣铐重获自由。
龙鳞甲 Dragon Scale Mail护甲（鳞甲），极珍稀（需同调）龙鳞甲是以某一种龙的鳞片制成的护甲。有时候是龙把自己褪下的鳞片送给类人生物，有时候则是猎人们小心的剥下死去龙类的鳞片并仔细保存起来。无论如何，龙鳞甲都是非常珍贵的物品。
...
屠龙者 Dragon Slayer武器（任意剑），珍稀你用该魔法武器发动的攻击检定和伤害掷骰获得+1 加值。
你以该武器命中龙类时，它将受到与剑的伤害类型相同的额外 3d6 伤害。此处的“龙”指任何生物类型为龙类的生物，包括龙龟 dragon turtle 和飞龙 wyvern 等。
漂浮之球 Driftglobe奇物，非普通这个厚玻璃制成的小球重 1 磅。在它周围 60 尺内，你可以说出命令语以使它施展出光亮术 light 或日光术 daylight。
...
无踪粉 Dust of Disappearance奇物，非普通这是一些装在小包里的细沙般的粉末，其分量足够使用一次。你可以用一个动作将粉末洒向空中，使得你和你周围10 尺内的生物和物件隐身 2d4 分钟（持续时间对所有受影响的对象一致）。粉末一经使用即被消耗。如果一名受影响的生物发动攻击或者施展法术，则该生物的隐形效应也随之终止。
干燥粉 Dust of Dryness奇物，非普通这个小包里共装着 1d6+4 把粉末。你可以用一个动作将一把粉末洒到水面上。粉末将把 15 尺立方区域的水变成一个弹珠大小的小球，漂浮或停止在粉末被洒下的位置附近。小球的重量可以忽略不计。
任一生物可以用一个动作将小球摔在某个结实表面，以令小球破碎并释放其中的水分。同时使小球失去其魔力。
身体大部分由水组成的元素生物在接触到一把干燥粉时必须进行一次 DC 13 的体质豁免，失败则受到 10d6 的黯蚀伤害，成功则伤害减半。
喷嚏粉 Dust of Sneezing and Choking奇物，非普通这种细沙般的粉末通常被装在小盒子里，其分量足够使用一次。这种粉末看起来像是无踪粉 dust of disappearance，对其施展鉴定术 identify 也会给出这样的结果。你可以用一个动作把这种粉末洒向空中。此时，你和你周围 30 尺内任何需要呼吸的生物必须进行一次 DC 15 的体质豁免，失败者将因不停打喷嚏而无法呼吸。受此影响的生物将陷入失能并窒息。
只要受影响的生物依然清醒，它就能在每次自己的回合结束时再次进行该豁免。一旦豁免成功，其自己身上的效应也将随之终止。法术次级复原术 lesser restoration 也可以终止一名生物身上的该效应。
矮人板甲 Dwarven Plate护甲（板甲），极珍稀着装该护甲时，你的 AC 获得+2 加值。此外，当某种效应将要在违背你意愿的情况下使你在地面上移动时，你可以用一个反应来减少你被移动的距离至多 10 尺。
矮人飞锤 Dwarven Thrower武器（战锤），极珍稀（需矮人同调）你用该魔法武器发动的攻击检定和伤害掷骰获得+3 加值。
它具有投掷属性，常规射程为 20 尺，最大射程为 60 尺。当你用此武器发动远程攻击命中时，它将造成 1d8 的额外伤害。
如果命中的目标是一名巨人，则额外伤害提升为 2d8。攻击后，此武器将立即飞回你的手中。
火巨灵囚瓶 Efreeti Bottle奇物，极珍稀这个彩绘的黄铜瓶子重 1 磅。如果你用一个动作拔出瓶塞，一团浓烟将从瓶内溢出。在你的回合结束时，该浓烟会随着一股无害的火焰消失，同时一名火巨灵 efreeti 将出现在你周边 30 尺内一个未被占据的空间位置。火巨灵的资料详见《怪物图鉴》。
瓶子第一次被打开时，DM 掷骰决定效应。
火巨灵链甲 Efreeti Chain护甲（链甲），传说（需同调）着装该护甲时，你的 AC 获得+3 加值，且免疫火焰伤害，还可以说和理解原初语 Primordial。此外，你还可以像在坚实地面上一样行走于熔岩上。
元素宝石 Elemental Gem奇物，非普通这种宝石蕴藏着操纵元素的能量。你可以用一个动作来打碎宝石，其效应如同你施展法术召唤元素生物 conjureelemental 一样召唤出一个元素。被打碎的宝石也将失去魔力。
宝石的种类决定了法术所召唤的元素。
健康灵药 Elixir of Health魔药，珍稀饮用此药水将治愈你身上的所有疾病，并移除目盲、耳聋、麻痹、中毒等状态。这种清澈的红色液体中有发光的小气泡。
精灵链甲 Elven Chain护甲（链甲衫），珍稀着装该护甲时，你的 AC 获得+1 加值。即使没有中型护甲的熟练项，你依然视为拥有该护甲的熟练项。
无尽烟壶 Eversmoking Bottle奇物，非普通这个重 1 磅，带铅塞的黄铜瓶子会渗出烟雾。如果你用一个动作拔掉瓶塞，一团浓重的云雾将从瓶中涌出，并扩散到瓶子周围 60 尺半径范围。被云雾笼罩的区域视处于重度遮蔽。
...
魅惑镜片 Eyes of Charming奇物，非普通（需同调）这种水晶镜片可以着装到眼睛上。魅惑镜片有 3 发充能。
着装该镜片时，如果你和你周围 30 尺内的一名类人生物能够互相看见对方，则你可以用一个动作并消耗 1 发充能来对它施展法术魅惑人类 charm person（DC 13）。镜片消耗掉的充能将在每天黎明时恢复。
微视镜片 Eyes of Minute Seeing奇物，非普通这种水晶镜片可以着装到眼睛上。着装该镜片时，可以更清晰的观察周边 1 尺以内的东西。在此范围内，你为搜索区域或研究物件所作，依赖视力的智力（调查）检定具有优势。
鹰眼镜片 Eyes of the Eagle奇物，非普通（需同调）这种水晶镜片可以着装到眼睛上。着装该镜片时，你依赖视力进行的感知（察觉）检定具有优势。在视野清晰的情况下，你可以在很远的地方看清小至 2 尺的生物或物件的细节。
异能塑像 Figurine of Wondrous Power奇物，稀有度依塑像种类而不同异能塑像是小到可以装在口袋里的野兽雕像。当你用一个动作说出命令语并把雕像扔到你周围 60 尺以内的某处时，它将变成一只活体生物。如果雕像被扔到的地方被其他生物或物件占据，或者周围的空间不足以容纳该雕像变成的生物，则它将不会变成生物。
雕像变成的生物对你和你的同伴态度友善。它能听懂你的语言并会按你的口头命令行事。如果你不下达任何命令，则该生物会保护自己而不作其他动作。除巨苍蝇 giant fly 外，其他生物的资料详见《怪物图鉴》。
巨苍蝇 Giant Fly大型野兽，无阵营AC：11HP：19（3d10+3）速度：30 尺，飞行 60 尺力量 14（+2） 敏捷 13（+1） 体质 13（+1）智力 2（-4） 感知 10（+0） 魅力 3（-4）感官：黑暗视觉 60 尺、被动察觉 10语言：—生物存在的时间依雕像而不同。持续时间结束时，生物将变回异能塑像形态。它的生命值降至 0，或你用一个动作碰触它并说出命令语时，它将提前变回雕像状态。变回雕像后，异能塑像的属性必须经过一段时间后才能再次启动，时间长短依雕像种类而不同。青铜狮鹫 Bronze Griffon（珍稀）。一尊青铜制成的扬爪狮鹫雕像，可以变为一只狮鹫 griffon，其存在至多维持 6 小时。使用后，此雕像需要经过 5 日才能再次启动。乌木苍蝇 Ebony Fly（珍稀）。一尊外形类似虻的乌木雕像，可以变为供骑乘的巨苍蝇 giant fly，其存在至多维持 12小时。使用后，此雕像需要经过 2 日才能再次启动。黄金狮子 Golden Lions（珍稀）。这尊黄金制成的狮子雕像总是成对制造。你可以只使用其中一只，也可以两只一起使用。每只狮子雕像都可以变为一只狮子 lion，其存在至多维持1 小时。每只狮子雕像都需要经过 7 日才能再次启动。象牙山羊 Ivory Goats（珍稀）。这种象牙制成的山羊雕像总是三个为一组制造。每个山羊雕像都有独特的外形和功能，如下所述：
• 旅行山羊 goat of traveling 可以变为一只资料和乘用马riding horse 相同的大型山羊。它有 24 发充能，处于野兽形态下时每小时（不足 1 小时按 1 小时计）将消耗 1 发充能。只要雕像还有充能，你就能随意使用它。当充能耗尽时，它将变回雕像且需要等到 7 日后充能恢复时才能再次启动。
...
焰舌 Flame Tongue武器（任意剑），珍稀（需同调）你可以用一个附赠动作说出命令语，以让这把剑的锋刃吐出烈焰。剑身的火焰为周围 40 尺范围提供明亮光照，以及额外 40 尺的微光光照。剑身燃烧时将对其命中的目标造成额外 2d6 的火焰伤害。火焰将持续燃烧到你把剑放下、收入鞘内或再次用一个附赠动作说出命令语为止。
折叠船 Folding Boat奇物，珍稀该物件外观看起来是个长 12 寸，宽 6 寸，深 6 寸的木盒。它重 4 磅且能在水面上漂浮，也可以像普通木盒一样装东西。该物品有三个命令语，说出一个命令语需要一个动作。
...
霜铭 Frost Brand武器（任意剑），极珍稀（需同调）当你用此魔法剑命中目标时，目标将受额外 1d6 的冷冻伤害。另外，握持此剑时你对火焰伤害具有抗性。
在严寒环境中，此剑的剑刃会发光，为周围 10 尺范围提供明亮光照，以及其外 10 尺的微光光照。
拔出此剑时，你可以选择让周围 30 尺内的所有非魔法火焰熄灭。该属性可以每小时使用一次。
食人魔力量护手 Gauntlets of Ogre Power奇物，非普通（需同调）着装该护手时，你的力量值变为 19。如果未着装该护手时你的力量值已经大于或等于 19，则该护手无效。
光彩夺目宝石 Gem of Brightness奇物，非普通这块棱形宝石有 50 发充能。持握该宝石时，你可以用一个动作说出宝石的三个命令语之一并造成如下效应：
...
真视宝石 Gem of Seeing奇物，珍稀（需同调）这块宝石有 3 发充能。你可以用一个动作说出宝石的命令语并消耗 1 发充能。接下来的 10 分钟内，你在透过宝石观察周围时具有 120 尺的真实视觉。该宝石在每天黎明时恢复1d3 发充能。
巨人屠杀者 Giant Slayer武器（任意斧或剑），珍稀你用此魔法武器发动的攻击检定和伤害掷骰获得+1 加值。
当你以此武器命中巨人 giant 时，它将额外受与武器伤害类型相同的 2d6 点伤害，并且必须成功通过一次 DC 15 的力量豁免否则应击倒地。此处的“巨人”指任何生物类型为巨人的生物，包括双头巨人 ettin 和巨魔 troll 等。
幻惑镶钉皮甲 Glamoured Studded Leather护甲（镶钉皮甲），珍稀着装该护甲时，你的 AC 获得+1 加值。你可以用一个附赠动作说出该护甲的命令语，使护甲的外观幻化为任何普通衣物或其他种类护甲。护甲的颜色，样式和配件都由你自己决定，但它的体积和重量不会改变。幻化外观的效应持续到你脱掉此护甲或再次改变护甲外观。
辟矢夺箭手套 Gloves of Missile Snaring奇物，非普通（需同调）着装这双手套时，它几乎和你的手融为一体。着装该手套且至少空着一只手时，如果远程武器攻击命中你，则你可以用反应来减少 1d10＋你的敏捷调整值的伤害。如果你把伤害减少到 0，则你可以选择抓住击中你的抛掷物（前提是它的尺寸足够让你用手抓住）。
运动健将手套 Gloves of Swimming and Climbing奇物，非普通（需同调）着装该手套时，攀爬和游泳不会消耗额外移动力。你进行攀爬与游泳相关的力量（运动）检定时获得+5 加值。
窃贼手套 Gloves of Thievery奇物，非普通这双手套着装到手上时是隐形的。着装该手套时，你进行的敏捷（巧手）检定，以及为开锁进行的敏捷检定具有+5 加值。
夜视镜 Goggles of Night奇物，非普通着装这副暗色护目镜时，你具有 60 尺黑暗视觉。如果你已经有黑暗视觉，则其范围再扩大 60 尺。
雷神之锤 Hammer of Thunderbolts武器（巨锤），传说你用此魔法武器发动的攻击检定和伤害掷骰获得+1 加值。
...
易容帽 Hat of Disguise奇物，非普通（需同调）着装这顶帽子时，你可以用一个动作随意施展易容术disguise，其效应持续至帽子被摘下为止。
智力头带 Headband of Intellect奇物，非普通（需同调）着装此头带时，你的智力值变为 19。如果未着装该头带时你的智力值已经大于或等于 19，则此头带无效。
光辉头盔 Helm of Brilliance奇物，极珍稀（需同调）这顶耀眼的头盔上镶嵌着 1d10 颗钻石 diamonds、2d10颗红宝石 rubies、3d10 颗火蛋白石 fire opals 和 4d10 颗蛋白石 opals。从头盔上摘下的宝石将化为尘埃。所有宝石都被摘下或摧毁后，此头盔将失去魔力。
着装这顶头盔时，你获得以下增益：
...
通晓语言头盔 Helm of Comprehending Languages奇物，非普通着装这顶头盔时，你可以用一个动作随意施展通晓语言comprehend language。
心灵感应头盔 Helm of Telepathy奇物，非普通（需同调）着装这顶头盔时，你可以用一个动作施展侦测思想 detectthoughts（豁免 DC 13）。保持法术专注期间，你可以用一个附赠动作向你聚焦的生物发送心灵通信。只要你依然聚焦于该生物，则它也可以用一个附赠动作回复你的信息。
使用侦测思想 detect thoughts 并聚焦于某生物期间，你还可以用一个动作对该生物施展暗示术 suggestion（豁免 DC13）。此后直到次日黎明前，其暗示术属性都无法再次启动。
传送头盔 Helm of Teleportation奇物，珍稀（需同调）这顶头盔有 3 发充能。着装这顶头盔时，你可以用一个动作并消耗 1 发充能施展传送术 teleport。每天黎明时头盔将恢复 1d3 发已消耗的充能。
霍华德的便利袋 Heward's Handy Haversack奇物，珍稀这个背包有三个袋子：中间一个侧面两个。每个袋都是一个异次元空间。每个侧袋可以容纳重量不超过 20 磅，体积不超过 2 立方尺的物体。中间的大袋则可以容纳重量不超过 80磅，体积不超过 8 立方尺的物质。无论装了多少东西，背包总是重 5 磅。
...
神圣复仇者 Holy Avenger武器（任意剑），传说（需圣武士同调）你用此魔法武器发动的攻击检定和伤害掷骰获得+3 加值。
...
音爆号角 Horn of Blasting奇物，珍稀你可以用一个动作说出号角的命令语然后吹响号角。号角将产生 30 尺锥状范围的音爆，其声音可传播远达 600 尺。
...
瓦尔哈拉号角 Horn of Valhalla奇物，珍稀（白银或黄铜）、极珍稀（青铜）、传说（黑铁）你可以用一个动作吹响号角。作为回应，约瑟园 Ysgard的战士英灵们将在你周围 60 尺内出现。这些英灵使用《怪物图鉴》上狂战士 berserker 的资料。生命值降至 0，或者 1 小时之后英灵们将返回约瑟园。使用后，号角需要经过 7 日才能再次启动。
...
西风马蹄铁 Horseshoes of a Zephyr奇物，极珍稀这些马蹄铁四个为一套。当一匹马或类似的生物同时着装四个马蹄铁时，它将悬浮在离地面 4 尺的高度，并如同在平地上一样移动。这使得该生物可以穿过或站在非固体或非稳定的平面上，例如水或岩浆。它无视困难地形并且不会留下足迹。另外，它也可以以正常速度连续移动 12 小时而不会因为强行军 dorced march 而疲乏。
加速马蹄铁 Horseshoes of Speed奇物，珍稀这些马蹄铁四个为一套。当一匹马或类似的生物同时着装四个马蹄铁时，它的步行速度将增加 30 尺。
不动权杖 Immovable Rod权杖，非普通这根铁制权杖的一端设有一按钮。如果你用一个动作按下按钮，权杖的魔法将把它固定在原地。权杖将保持不动（即使这违反重力）直到你或另一名生物用一个动作再次按下按钮。权杖最多能负载 8,000 磅的重量，超过则权杖的效应自动关闭并掉落。任一生物可以用一个动作进行一次 DC 30 的力量检定，成功则可以将权杖移动至多 10 尺。
吟游诗人乐器 Instrument of the Bards奇物，多种稀有度（需吟游诗人同调）吟游诗人乐器是乐器中的精品，在各方面都优于普通乐器。这类乐器共有七种，每种都以一个传奇吟游诗人学院的名字命名。下表列出了每种乐器的稀有度、乐器间共通的法术以及每种乐器独有的法术。未与一件吟游诗人乐器同调的生物在尝试弹奏它时必须成功通过一次 DC 15 的感知豁免否则将受 2d4 的心灵伤害。
...
艾恩石 Ioun Stone奇物，多种稀有度（需同调）艾恩 Ioun 在许多世界被尊为知识和预言之神，而其名字则被用来命名这种石头。
...
比拉罗的铁索 Iron Bands of Bilarro奇物，稀有这个锈迹斑斑的铁球重 1 磅，直径 3 寸。你可以用一个动作说出命令语并把铁球扔向一个 60 尺内你能看到的巨型或更小体型生物。球在空中飞行时会张开成为一张铁网。
...
收妖瓶 Iron Flask奇物，传说这是一个带黄铜瓶塞的铁瓶。你可以选择一个 60 尺内你能看见的生物作为目标，并用一个动作说出瓶子的命令语。如果目标并非你所处位面的生物，则它必须成功通过一次 DC 17的感知豁免否则将被困入瓶中。如果目标曾经被瓶子囚禁过，则它进行的豁免具有优势。一旦被瓶子吸入，该生物将被困在瓶中直到被释放。收妖瓶最多只能同时容纳一名生物。被困在瓶中的生物无需呼吸和饮食，也不会衰老。
...
闪电标枪 Javelin of Lighting武器（标枪），非普通这把标枪是魔法武器。当你投掷它并说出命令语时，它将转变为一道 5 尺宽的闪电束，向 120 尺内的某个目标飞去。
...
卡夫统灵药 Keoghtom's Ointment奇物，非普通这个直径 3 寸的玻璃瓶中装着 1d4+1 剂闻起来隐约有芦荟气味的混合剂。瓶子和混合剂共重 1/2 磅。
用一个动作可以吞下或外敷一剂软膏。软膏将为使用它的生物恢复 2d8+2 的生命值，并解除毒素和治愈任何疾病。
显像提灯 Lantern of Revealing奇物，非普通这盏带罩提灯点燃时提供 30 尺的明亮光照和其外 30 尺的微光光照。1 品脱油可以供提灯燃烧 6 小时。处于提灯明亮光照范围内的隐形生物或物件将被其显形。你可以用一个动作降下灯罩，让提灯仅在 5 尺内提供微光光照。
吉兆之刃 Luck Blade武器（任意剑），传说（需同调）你用该魔法武器发动的攻击检定和伤害掷骰获得+1 加值。
...
瓦解之锤 Mace of Disruption武器（硬头锤），珍稀（需同调）当你用这把魔法武器命中一名邪魔或不死生物时，该目标将额外受 2d6 的光耀伤害。受此伤害后该生物的生命如果小于等于 25，则它必须成功通过一次 DC 15 的感知豁免否则被摧毁。如果豁免成功，则直到你的下一回合结束，该生物对你恐慌。
你握持此武器时，它将向 20 尺范围散发出明亮光照以及其外 20 尺的微光光照。
打击之锤 Mace of Smiting武器（硬头锤），珍稀你在用此魔法武器发动的攻击检定和伤害掷骰获得+1 加值。如果你使用此武器攻击构装体，则加值变为+3。
当你使用这把武器攻击并投出 20 时，目标将额外受 7 点钝击伤害。如果目标是构装体，则额外伤害为 14。如果受到此伤害后该构装体的生命值小于等于 25，则目标被摧毁。
恐惧之锤 Mace of Terror武器（硬头槌），珍稀（需同调）这把魔法武器有 3 发充能。持握它时，你可以用一个动作并消耗一发充能来释放一股恐惧波动。你周围 30 尺半径内每个被你指定的生物必须成功通过一次 DC 15 的感知豁免否则对你恐慌 1 分钟。受该恐慌影响的生物必须在自己的回合尽全力向远离你的方向移动，且无法主动进入你周围 30 尺范围内也无法执行反应。它的只能选择疾走 Dash 动作或是试图脱离阻止它移动的效果。如果无处可逃，则它可以使用闪避Dodge 动作。每个该生物的回合结束时，它都可以再次进行该豁免，成功则终止其自己身上的该效应。
每天黎明时恐惧之锤将恢复 1d3 发充能。
法术抗性斗篷 Mantle of Spell Resistance奇物，珍稀（需同调）穿着这件斗篷时，你对法术的豁免具有优势。
强身手册 Manual of Bodily Health奇物，极珍稀这本手册记载了保健与饮食的秘诀，其文字蕴含魔力。如果你在 6 日或更少的时间内花费 48 小时研读这本书并实践其中的方针，你的体质属性和体质属性上限均增加 2。之后这本手册将失去魔力，直到一个世纪后才能恢复。
健体手册 Manual of Gainful Exercise奇物，极珍稀这本手册记载了健身与运动的秘诀，其文字蕴含魔力。如果你在 6 日或更少的时间内花费 48 小时研读这本书并实践其中的方针，你的力量属性和力量属性上限均增加 2。之后这本手册失去魔力，直到一个世纪后才能恢复。
魔像手册 Manual of Golems奇物，极珍稀这本手册记载了制造某种魔像所必须的知识和符咒，魔像的类型由 DM 选择或随机决定。只有拥有至少 2 个 5 环法术位的施法者才能解读并使用这本书。无法解读这本书的生物在阅读它时将受到 6d6 的心灵伤害。
...
灵巧手册 Manual of Quickness of Action奇物，极珍稀这本手册记载了平衡与协调的秘诀，其文字蕴含魔力。如果你在 6 日或更少的时间内花费 48 小时研读这本书并实践其中的方针，你的敏捷属性和敏捷属性上限均提升 2。之后这本手册失去魔力，直到一个世纪后才能恢复。
水手护甲 Mariner's Armor护甲（轻型，中型或重型），非普通着装该护甲时，你获得等同于你步行速度的游泳速度。另外，当你在水下以 0 生命值开始你的回合时，护甲将使你向水面上升 60 尺。护甲表面带着鱼贝的纹饰。
读心勋章 Medallion of Thoughts奇物，非普通（需同调）这枚勋章有 3 发充能。着装勋章时，你可以用一个动作并消耗 1 发充能使用侦测思想 detect thoughts（DC 13）。勋章每天黎明时恢复 1d3 发充能。
摄心镜 Mirror of Life Trapping奇物，极珍稀以非直接的方式看向这面镜子时，其中映着生物模糊的影像。镜子重 50 磅，拥有 AC11，生命值为 10，钝击易伤。
生命值降至 0 时，镜子将粉碎并被摧毁。
...
秘银甲 Mithral Armor护甲（中型或重型，非兽皮甲），非普通秘银是一种轻盈柔韧的金属。秘银链甲衫或胸甲可以穿在普通衣物下面。一些护甲会使着装者的敏捷（隐匿）检定具有劣势，或是对穿着者的力量属性有要求，但它的秘银版本则没有这些特性。
适应项链 Necklace of Adaptation奇物，非普通（需同调）卓专该项链时，你可以在任何环境中正常呼吸，并且其对抗有害气体（如死云术 cloudkill、臭云术 stinking cloud、吸入式毒药和某些龙的吐息武器）时进行的豁免具有优势。
火球项链 Necklace of Fireballs奇物，珍稀这条项链上挂着 1d6+3 颗珠子。你可以用一个动作取下一颗珠子并投掷到至多 60 尺远的地方。它将在移动轨迹的终点爆炸，效果等同于 3 环的火球术 fireball（豁免 DC 15）。
你可以一次投掷多个珠子甚至整条项链。同时投掷的珠子数量每增加 1，火球术的环阶也增加 1。
念珠项链 Necklace of Prayer Beads奇物，珍稀（需牧师、德鲁伊或圣武士同调）这条项链上挂着蓝晶 aquamarine、黑珍珠 black pearl 以及黄玉 topaz 制成的 1d4+2 颗魔法珠子。另外也有许多非魔法的珠子挂在项链上，例如琥珀 amber、血石 bloodstone、黄水晶 citrine、珊瑚 coral、翡翠 jade、珍珠 pearl 和石英 quartz。
...
九转夺命剑 Nine Lives Stealer武器（任意剑），极珍稀（需同调）你用此魔法武器发动的攻击检定和伤害掷骰获得+2 加值。
...
诺泽尔的惊奇颜料 Nolzur's Marvelous Pigments奇物, 极珍稀这种颜料通常 1d4 罐一组和画笔一起装在精美的木盒里（共重 1 磅）。它可以把你在二维平面上画出的东西变为三维实体。当你用心画下图案时，它将从笔尖跃动而出，形成你想要的形状。
...
誓仇弓 Oathbow武器（长弓），极珍稀（需同调）当你弯弓搭箭时，誓仇弓会用精灵语悄声念出“速歼敌”。
...
以太化之油 Oil of Etherealness魔药，珍稀这种浑浊的灰色油膏在容器外会变成珠状并迅速蒸发。
一瓶油膏可以涂抹一个中型或更小体型的生物及其着装和携带的装备上。生物的体型每比中型大一级就需要多使用一瓶。
使用以太化之油耗时 10 分钟，此后受油膏影响的生物将获得法术以太化 etheralness 的效应，并持续 1 小时。
锐锋之油 Oil of Sharpness魔药，极珍稀这种透明的胶状油膏中掺杂着极纤细的银色碎片。它可以涂抹于一把挥砍或穿刺武器，或是 5 发造成挥砍或穿刺伤害的弹药上。使用锐锋之油耗时 1 分钟，此后受油膏影响的武器或弹药所发动的攻击检定和伤害掷骰获得+3 加值。
滑溜之油 Oil of Slipperiness魔药，非普通这种粘稠的黑色油膏装在容器中时看起来十分浓重，但倒出来时却可以迅速流动。油膏可以涂抹于一个中型或更小体型的生物及其着装和携带的装备上。生物的体型每比中型大一级就需要多使用一瓶。使用滑溜之油耗时 10 分钟，此后受油膏影响的生物获得自由行动 freedom of movement 的效应，并持续 8 小时。
你也可以用一个动作把这瓶油膏泼在 10 平方尺的地面上。该区域将如同被施展了法术油腻术 grease，并持续 8 小时。
法力再生珍珠 Pearl of Power奇物，非普通（需施法者同调）该珍珠在你身上时，你可以用一个动作说出珍珠的命令语以恢复一个已消耗的法术位。如果已消耗的法术位为 4 环或更高环阶，则所恢复的法术位为 3 环。使用后直到次日黎明前，珍珠都无法再次启动。
保健护符 Periapt of Health奇物，非普通着装此护符时，你免疫所有疾病。如果你在此前已经感染了疾病，则疾病的效应在你着装此护符期间被压制。
辟毒护符 Periapt of Proof Against Poison奇物，珍稀这条精巧的银链上挂着一颗切割闪亮的黑宝石吊坠。着装它时，毒素无法影响你。你免疫中毒状态和毒素伤害。
愈合护符 Periapt of Wound Closure奇物，非普通（需同调）着装此护符时，如果你在自己的回合开始时处于濒死状态，则你将直接转为伤势稳定。此外，你掷生命骰恢复生命值时，其恢复的数值加倍。
迷情媚药 Philter of Love魔药，非普通饮用此药水后 10 分钟内，你将被你看见的第一名生物魅惑。魅惑效应持续 1 小时。如果该生物的物种和性别符合你的取向，被魅惑期间你将把它视为自己的真爱。不仔细观察的话，很难发现这种漂浮着气泡的玫瑰色液体中有一个气泡是心形的。
颤栗乐笙 Pipes of Haunting奇物，非普通你必须拥有乐器的熟练项才能使用该乐笙。该乐笙有 3发充能。你可以用一个动作吹奏它并消耗 1 发充能，以发出一阵含有魔法的怪异音调。你周围 30 尺内的每名生物都必须成功通过一次 DC 15 的感知豁免否则对你恐慌 1 分钟。你可以选择让范围内所有对你没有敌意的生物豁免自动成功。豁免失败的生物可以在每次它的回合结束时再次进行该豁免，成功则终止其自己身上的恐慌效应。豁免成功的生物在 24 小时内免疫该乐笙的效应。该乐笙在每天黎明时恢复 1d3 发充能。
唤鼠乐笙 Pipes of the Sewers奇物，非普通（需同调）你必须拥有乐器的熟练项才能使用该乐笙。与唤鼠乐笙同调时，普通老鼠和巨鼠对你的态度变为冷漠。除非你威胁或伤害它们，否则它们不会主动攻击你。
...
以太化板甲 Plate Armor of Etherealness护甲（板甲），传说（需同调）着装该护甲时，你可以用一个动作说出命令语以获得法术以太化 etherealness 的效应，并持续 10 分钟，或直到你用一个动作再次说出命令语，亦或持续至你脱下此护甲。此后直到次日黎明前，该属性都无法再次启动。
次元洞 Portable Hole奇物，珍稀这块丝绸般顺滑的黑色布料展开时为直径 6 寸的圆形，也可以被折叠成手帕大小。
...
化兽为友药水 Potion of Animal Friendship魔药，非普通饮用此药水后 1 小时，你可以任意施展化兽为友 animalfriendship（DC 13）。搅动这种浑浊的液体可以看到一些残渣：
鱼鳞、蜂鸟的舌头、猫爪或松鼠毛。
鹰眼术药水 Potion of Clairvoyance魔药，珍稀饮用此药水后，你获得鹰眼术 clairvoyance 的效应。这种泛黄的液体中漂浮着一个眼球，但会在你打开瓶盖时消失。
攀爬药水 Potion of Climbing魔药，普通饮用此药水后，你获得等同于你步行速度的攀爬速度，持续 1 小时。在此期间，你攀爬时进行的力量（运动）检定具有优势。药水分离成类似于岩层的褐色、银色和灰色三层，即使摇晃也不会让三种颜色混合到一起。
缩小药水 Potion of Diminution魔药，珍稀饮用此药水后，你获得变巨/缩小术 enlarge/reduce 的“缩小”效应，并持续 1d4 小时（无需专注）。药水中的红色成分不断聚合成一个小球，然后又散开将周围的液体染成红色。摇晃药水也不会打断这个过程。
火焰吐息药水 Potion of Fire Breath魔药，非普通饮用此药水后，你可以用一个附赠动作向你周边 30 尺内的一个目标喷出火焰。目标必须进行一次 DC 13 的敏捷豁免，失败受到 4d6 的火焰伤害，成功则伤害减半。药水的效应在你喷出三次火焰或 1 小时后终止。
这瓶橙色的药水不停闪烁着，瓶中没有药水的部分则充满了烟雾。打开容器时烟雾将飘出。
飞行药水 Potion of Flying魔药，极珍稀饮用此药水后，你获得等同于你步行速度的飞行速度并可以悬浮，效应持续 1 小时。如果药水效应终止时你仍在空中，并且也没有其他飞行手段，那么你将坠落。这种透明的液体漂浮在容器顶部，其中含有些许云雾状的白色杂质。
气化形体药水 Potion of Gaseous Form魔药，珍稀饮用此药水后，你获得气化形体 gaseous form 的效应，持续一小时或直到你用一个附赠动作将其终止（无需专注）。
这种药水装在瓶中若云雾，倒出来则似水流。
巨人之力药水 Potion of Giant Strength魔药，多种稀有度饮用此药水后，你的力量值将变为药水所赋予的数值（由巨人类型决定，见下表），持续 1 小时。如果你的力量值已经大于或等于药水所能赋予的数值，则药水无效。
巨人之力药水透明的药液中漂着一点对应类型巨人的指甲碎片。
霜巨人之力药水和石巨人之力药水效果相同。
巨人之力药水 Potion of Giant Strength魔药，多种稀有度饮用此药水后，你的力量值将变为药水所赋予的数值（由巨人类型决定，见下表），持续 1 小时。如果你的力量值已经大于或等于药水所能赋予的数值，则药水无效。
巨人之力药水透明的药液中漂着一点对应类型巨人的指甲碎片。
霜巨人之力药水和石巨人之力药水效果相同。
英雄气概药水 Potion of Heroism魔药，珍稀饮用此药水后，你获得 10 点临时生命值和祝福术 Bless的效应（无需专注），持续 1 小时。这种蓝色的药水如同沸腾搬冒出气泡和蒸汽。
隐身药水 Potion of Invisibility魔药，极珍稀装着这种药水的容器看起来是空的，但你仍能感觉到其中保存着液体。饮用此药水后，你隐形 1 小时。你着装或携带的东西也将和你一起隐形。如果你攻击或施法，则隐形效应提前终止。
坚不可摧药水 Potion of Invulnerability魔药，珍稀饮用此药水后，你获得对所有伤害的抗性，持续 1 分钟。
这种粘稠的药水看起来就像液化的钢铁。
延寿药水 Potion of Longevity魔药，极珍稀饮用此药水后，你的生理年龄减少 1d6+6 岁，但不能小于 13 岁。此后每当你再次饮这种药水时，都会累加 10%的概率令你增加而非减少 1d6+6 岁。这种琥珀色药水中悬浮着一条蝎子的尾巴，一颗蝰蛇的牙，一只死蜘蛛，以及一颗违反常理地跳动着的小心脏。药水被打开时，这些成分便随之消失。
读心药水 Potion of Mind Reading魔药，珍稀饮用此药水后，你获得侦测思想 detect thoughts（DC 13）的效应。这种粘稠的紫色药水中漂浮着一团粉色的卵形云雾。
剧毒药水 Potion of Poison魔药，非普通这种药水看起来、闻起来和尝起来都和治疗药水 potionof healing 或其他增益药水差不多。但实际上，它是用幻术伪装过的毒药。
...
抗性药水 Potion of Resistance魔药，非普通饮用此药水时，你获得对某种伤害类型的抗性，持续 1 小时。DM 可以自行选定伤害类型，或从下表中随机选择。
加速药水 Potion of Speed魔药，极珍稀饮用此药水后，你获得加速术 haste 的效应，持续 1 分钟（无需专注）。这瓶黄色的药水中混杂着黑色杂质，并有一个漩涡不断自行旋转。
活力药水 Potion of Vitality魔药，极珍稀饮用此药水将消除你所有的力竭状态，并治愈影响你的任何疾病或毒素。此后 24 小时内，你使用生命骰时所恢复的生命值都为生命骰的最大值。这种深红色药水有节奏的发出微光，让人联想到心脏的跳动。
水下呼吸药水 Potion of Water Breathing魔药，非普通饮用此药水后，你可以在水下呼吸，持续 1 小时。这种浑浊的绿色药水闻起来有海水的味道，其中漂浮着一个水母形状的气泡。
夸尔羽符 Quaal's Feather Token奇物，珍稀这是一件外观类似羽毛的小物品。夸尔羽符有很多种，每种都有一个一次性效应。羽符的类型由 DM 选取或参照下表随机决定。
锚 Anchor。你可以用一个动作以这枚羽符碰触一艘船。
接下来的 24 小时中，无论采取何种方式，该船都无法被移动。
用羽符再次碰触该船将终止该效应。效应终止时，羽符也将随之消失。
鸟 Bird。你可以用一个动作将这枚羽符抛到 5 尺高的空中。羽符将消失，取而代之的是一只五彩斑斓的大鸟。这只鸟使用大鹏 roc 的资料（详见《怪物图鉴》）。它服从你的简单命令但无法攻击。负载不超过 500 磅时，它能以全速飞行（16里每小时，每飞行 3 小时休息 1 小时，每天至多飞行 144 里）。
而负重不超过 1000 磅时则以半速飞行。大鸟飞过 144 里或生命值降至 0 后消失。你可以用一个动作解消羽符的效应。
...
艾罗娜的箭袋 Quiver of Ehlonna奇物，非普通这个箭袋有三个口袋，每一个都连接着一个异次元空间。
...
羽符将消失，取而代之的是一条悬浮在空中的长鞭。你可以用一个附赠动作让长鞭对它周围 10 尺内的一名生物发动近战法术攻击，攻击加值+9。如果攻击命中，目标将受到 1d6+5的力场伤害。
你可以在自己的回合里用一个附赠动作命令长鞭飞行至多 20 尺并攻击它周围 10 尺内的一名生物。长鞭将在 1 小时后消失，你陷入失能或你死亡时鞭子也会消失。你可以用一个动作解消羽符的效应。
支配动物戒指 Ring of Animal Influence戒指，珍稀这枚戒指有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。着装这枚戒指时，你可以用一个动作并消耗 1 发充能以施展下列法术其一：
...
命令元素戒指 Ring of Elemental Command戒指，传说（需同调）这枚戒指连接着四个元素位面之一，由 DM 决定或随机选择。
...
反射闪避戒指 Ring of Evasion戒指，珍稀（需同调）这枚戒指有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。着装这枚戒指时，如果你进行敏捷豁免失败，则你可以用你的反应消耗 1 发充能使该豁免成功。
羽落戒指 Ring of Feather Falling戒指，珍稀（需同调）着装这枚戒指时，你在坠落中每回合只会下降 60 尺，并且不会受到坠落伤害。
自由行动戒指 Ring of Free Action戒指，珍稀（需同调）着装这枚戒指时，你通过困难地形时无需消耗额外的移动力。另外，魔法无法降低你的速度，也无法令你陷入麻痹或束缚状态。
隐身戒指 Ring of Invisibility戒指，传说（需同调）着装这枚戒指时，你可以用一个动作进入隐形。任何你着装或携带的东西也将随你隐形。你保持隐形直到发动攻击、施展法术，或是戒指被摘下。你也可以用一个附赠动作解除隐形。
跳跃戒指 Ring of Jumping戒指，非普通（需同调）着装这枚戒指时，你可以随意以一个附赠动作施展跳跃术 jump，但只能以自身为目标。
心灵护盾戒指 Ring of Mind Shielding戒指，非普通（需同调）着装这枚戒指时，你免疫其他生物对你使用的以下魔法效应：读心、测谎、侦测阵营、侦测生物类型。只有在你同意时，其他生物才能和你建立心灵感应。
...
防御戒指 Ring of Protection戒指，珍稀（需同调）着装这枚戒指时，你的 AC 和豁免获得+1 加值。
再生戒指 Ring of Regeneration戒指，极珍稀（需同调）着装这枚戒指时，只要你尚有至少 1 点生命值，你就能每 10 分钟恢复 1d6 的生命值。即使你失去了一部分肢体，只要你能在 1d6+1 日内保持至少有 1 点生命值，则失去的肢体也会重新长出并恢复功能。
抗性戒指 Ring of Resistance戒指，珍稀（需同调）着装该戒指时，你获得对某种类型伤害的抗性。戒指上镶嵌的宝石种类决定抗性的种类，具体由 DM 决定或随机选择。
流星戒指 Ring of Shooting Stars戒指，极珍稀（需在夜晚的户外同调）着装这枚戒指且在黑暗或微光光照环境下时，你可以随意施展舞光术 dancing light 和光亮术 light。利用戒指施法需要一个动作。
...
储法戒指 Ring of Spell Storing戒指，珍稀（需同调）这枚戒指可以储存对它施展的法术，保存直到与它同调的生物使用这些法术。戒指中可以储存的法术环数之和不能超过 5。新发现的储法戒指可能已经储存了由 DM 选定 1d6-1个环阶的法术。
...
法术反转戒指 Ring of Spell Turning戒指，传说（需同调）着装这枚戒指时，你在对任何仅以你为目标（而非影响你所在区域）的法术进行豁免时具有优势。另外，该法术为 7 环或更低环阶且你的豁免骰出 20 时，该法术对你无效且目标变为原施法者。使用原施法者的资料决定法术位环阶、豁免 DC、攻击加值和施法关键属性属性。
善泳戒指 Ring of Swimming戒指，非普通着装这枚戒指时，你获得 40 尺游泳速度。
心灵遥控戒指 Ring of Telekinesis戒指，极珍稀（需同调）着装这枚戒指时，你可以随意施展心灵遥控 telekinesis，但只能以未被着装或携带的物件为目标。
公羊戒指 Ring of the Ram戒指，珍稀（需同调）这枚戒指有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。着装这枚戒指时，你可以用一个动作并消耗 1 到 3发充能以攻击你周围 60 尺内一名你能看见的生物。戒指产生一个山羊头的幻影冲向目标，其攻击加值为+7。如果攻击命中，每消耗 1 发充能，目标就受到 2d10 的力场伤害并被向远离你的方向推开 5 尺。
此外，你还可以用一个动作并消耗 1 到 3 发充能以破坏周围 60 尺内一件你能看到，且未被着装或携带的物件。每消耗 1 发充能，戒指就在力量检定时获得一个+5 加值。
三愿戒指 Ring of Three Wishes戒指，传说这枚戒指有 3 发充能。着装这枚戒指时，你可以用一个动作并消耗 1 发充能施展祈愿术 wish。充能用尽后，这枚戒指将变为非魔法物品。
温暖戒指 Ring of Warmth戒指，非普通（需同调）着装这枚戒指时，你对寒冷伤害具有抗性。另外，你自身及你着装或携带的物件都可不受最低华氏-50 度的低温影响。
水行戒指 Ring of Water Walking戒指，非普通着装这枚戒指时，你可以在液体表面站立和行走，如同在坚实地面上一般。
X 射线戒指 Ring of X-Ray Vision戒指，珍稀（需同调）着装此戒指时，你可以用一个动作说出命令语启动戒指的效应。启动后，你获得可以看到实体物件内部或穿透它看到后方的特殊视觉，其半径为 30 尺，持续 1 分钟。对你而言，此视觉范围内的实体均为透明状态，并且不会阻碍光线通过。
此视觉可以穿透 1 尺厚的石头、1 寸厚的普通金属、或是至多3 尺厚的木头或泥土。更厚的物件，或是一层铅片，将阻碍这种视觉。
如果你在一次长休前尝试再次启动这枚戒指，则必须成功通过一次 DC 15 的体质豁免否则将提升一级力竭。
百眼法袍 Robe of Eyes奇物，珍稀（需同调）这件长袍上绘满了眼睛的图案。着装这件长袍时，你获得以下好处：
...
法术反转戒指 Ring of Spell Turning戒指，传说（需同调）着装这枚戒指时，你在对任何仅以你为目标（而非影响你所在区域）的法术进行豁免时具有优势。另外，该法术为 7 环或更低环阶且你的豁免骰出 20 时，该法术对你无效且目标变为原施法者。使用原施法者的资料决定法术位环阶、豁免 DC、攻击加值和施法关键属性属性。
虹光法袍 Robe of Scintillating Colors奇物，极珍稀（需同调）这件长袍有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。
...
星辰法袍 Robe of Stars奇物，极珍稀（需同调）这件黑色和深蓝为底色的长袍上点缀着白色或银色的星星。着装这件长袍时，你的豁免具有+1 加值。
...
大法师法袍 Robe of the Archmagi奇物，传说（需术士、邪术师或法师同调）这件典雅的长袍由白色、灰色或黑色的精制布料制成，并饰有银色的符文。每一件大法师法袍都为某个阵营所定制，其颜色与阵营对应。白色代表善良、灰色代表中立，而黑色代表邪恶。你无法和与你阵营不符的法袍同调。
着装这件长袍时，你获得以下好处：
• 不着装护甲时，你的基础 AC 为 15＋你的敏捷调整值。
• 你对抗法术和其他魔法效果时作的豁免检定具有优势。
• 你的法术豁免 DC 和法术攻击加提升 2。
杂货法袍 Robe of Useful Items奇物，非普通这件长袍上打着各种颜色和形状的补丁。穿着这件长袍时，你可以用一个动作撕下一块补丁，令它变成它象征的物件。
所有补丁都被撕下后，这件长袍将变为普通衣物。
杂货法袍有以下补丁各 2 个：
• 匕首• 牛眼提灯（装满燃料且点燃）• 钢镜• 10 尺长杆• 麻绳（长 50 尺，盘绕状态）• 麻布袋另外，杂货法袍还有 4d4 个其他补丁。补丁对应的物品由 DM 选择或随机决定。
吸收权杖 Rod of Absorption权杖，极珍稀（需同调）持握这把权杖时，你可以用反应来吸收一个仅以你为目标（而非影响你所在的区域）的法术。被吸收的法术不产生效应，但其能量（而非法术本身）被权杖吸收。这股能量的环阶与该法术被施展时的环阶相同。吸收权杖从制成到毁灭只能储存共 50 环的能量。吸收满 50 环能量后，它将无法再吸收法术。如果权杖无法吸收某个以你为目标的法术，则该法术不受权杖影响。
...
警示权杖 Rod of Alertness权杖，极珍稀（需同调）这把权杖顶端有尖刺状凸起。权杖具有以下属性：
...
王者权杖 Rod of Lordly Might权杖，传说（需同调）这把权杖顶端有尖刺状凸起。它还是一把魔法硬头锤。你用它发动的攻击检定和伤害掷骰获得+3 加值。
...
契约掌控者权杖 Rod of the Pact Keeper权杖，非普通（+1），珍稀（+2），极珍稀（+3）（需邪术师同调）持握这把权杖时，你施展邪术师法术时法术攻击检定与豁免 DC 获得由权杖稀有度决定的加值。
此外，持握这把权杖时，你可以用一个动作恢复 1 个邪术师法术位。你必须完成一次长休才能再次使用权杖的这个属性。
复活权杖 Rod of Resurrection权杖，传说（需牧师、德鲁伊或圣武士同调）这把权杖有 5 发充能。持握权杖时，你可以用一个动作施 展 以 下 法 术 ：
医 疗 术 heal （ 消 耗 1 充 能 ） 或复 活 术resurrection（消耗 5 充能）。
权杖每天黎明时恢复 1 发已消耗的充能。当权杖充能耗尽时，骰一次 d20。如果掷骰结果为 1，则权杖在一阵强光中消失。
支配权杖 Rod of Rulership权杖，珍稀（需同调）你可以用一个动作展示权杖，并指定周边 120 尺内若干能看见的生物使其服从于你。每个目标必须成功通过一次 DC15 的感知豁免否则被你魅惑 8 小时。豁免失败的生物将视你为可以信赖的领袖。如果你或你的同伴伤害了被支配者，或是命令被支配者进行违反其天性的行为，魅惑将立即终止。此后直到次日黎明前，该权杖都无法再次启动。
庇护权杖 Rod of Security权杖，极珍稀持握这把权杖时，你可以用一个动作启动它。权杖立刻将你和你能看到的至多 199 名自愿的其他生物传送到存在于异位面的一处乐园福地。你可以自行定义乐园的景观。它可以是宁静的花园、迷人的林间空地、热闹的酒馆、宽阔的宫殿、热带的海岛、奇妙的嘉年华会，或是任何你能想象到的美景。无论环境如何，乐园都有足够的食物和水供访客享用。除食水之外，乐园内的任何东西都只能在乐园的异位面空间中存在。例如，在乐园的花园中采的花在被带出异位面空间时将消失。
...
攀爬绳 Rope of Climbing奇物，非普通这条 60 尺长的丝质绳索重 3 磅，可以承受 3000 磅的重量。你可以握住绳索的一端并用一个动作说出命令语以使绳索活化。以一个附赠动作，你可以命令绳索的另一端向你指定的目的地移动。从你下达命令的回合开始，绳索的另一端在你的每个回合移动 10 尺，直到到达目的地、长度不足或你命令它停止。你也可以命令绳索牢牢绑在一个指定物体上，或是松绑、打结、解结、卷起。
...
纠缠绳 Rope of Entanglement奇物，珍稀这条绳索长 30 尺，重 3 磅。你可以握住绳索的一端并用一个动作说出命令语，绳索的另一端将疾速飞出困住你周围20 尺内一名你能看见的生物。该生物必须成功通过一次 DC15 的敏捷豁免否则被束缚。
你可以用一个附赠动作说出另一命令语以释放被束缚的生物。被束缚的生物也可以用一个动作进行一次 DC 15 的力量检定或敏捷检定（由该生物选择），检定成功则挣脱束缚。
绳索拥有 AC 20 和 20 点生命值。只要还剩至少 1 点生命值，绳索就能每 5 分钟恢复 1 点生命值。如果绳索的生命值降至 0，则它被摧毁。
铁骑马鞍 Saddle of The Cavalier奇物，非普通骑乘配有这副马鞍的坐骑时，只要你保持清醒，你就不会在非自愿的情况下解除骑乘状态。同时，所有对你坐骑发动的攻击检定具有劣势。
防护圣甲虫 Scarab of Protection奇物，传说（需同调）如果你用手握住这个甲虫形徽章 1 轮，它表面上将浮现出铭文，揭示其魔法本质。当你携带圣甲虫时，它为你提供两项增益：
• 你在对法术进行的豁免具有优势。
• 圣甲虫有 12 发充能。如果你在对抗一个死灵系法术或由不死生物造成的有害效应时豁免失败，则你可以使用反应并消耗 1 发充能以使该豁免成功。充能耗尽后，防护圣甲虫将化为灰烬。
急速弯刀 Scimitar of Speed武器（弯刀），极珍稀（需同调）你用此魔法武器发动的攻击检定和伤害掷骰获得+2 加值。
另外，你在自己的每一回合里可以用一个附赠动作以该弯刀发动一次攻击。
守护卷轴 Scroll of Protection卷轴，珍稀每张守护卷轴 仅针对一种特定类型的生物起效。生物类型由 DM 指定或按下表随机决定。
...
传讯石 Sending Stones奇物，非普通传讯石由两块经过雕刻相互可以匹配的光滑石头组成，外形非常容易辨认。当你碰触其中一块传讯石时，你可以用一个动作施展短讯术 sending，目标为另一块传讯石的携带者。
如果另一块传讯石没有被任何生物携带，则你在使用石头时将会得知这一信息，而该短讯术 不会被浪费。
使用短讯术 后，直到次日黎明前传讯石都无法再次启动。
如果一块传讯石被摧毁，则与它配对的另一块石头也将失去其魔法。
警戒之盾 Sentinel Shield护甲（盾牌），非普通持握此盾牌时，你所进行的先攻检定和感知（察觉）检定具有优势。这面盾牌上绘制一个眼睛的图案。
盾牌 Shield，+1、+2 或+3护甲（盾牌），非普通（+1），珍稀（+2），极珍稀（+3）持握此盾牌时，你将依据盾牌本身的 AC 加值基础上获得额外的加值。加值的数值取决于该盾牌的稀有度。
引弹盾 Shield of Missile Attraction护甲（盾牌），珍稀（需同调）持握此盾牌时，你对远程武器造成的伤害具有抗性。诅咒 Curse。与这面受诅咒的盾牌同调的生物将受到其诅咒影响。摘下盾牌不会终止诅咒，只有对你施展移除诅咒remove curse 或类似的魔法才能移除。受诅咒影响时，任何瞄准你周围 10 尺内目标的远程武器攻击都将转为攻击你。
蛛行便鞋 Slippers of Spider Climbing奇物，非普通（需同调）着装这双便鞋时，你可以在垂直平面上移动，也可以在松开双手的情况下倒挂在天花板上。你获得等于你步行速度的攀爬速度。不过，这双鞋不能让你在滑溜的平面，如冰、油覆盖的平面上行走。
至尊胶 Sovereign Glue奇物，传说这种白浊粘稠的物质可以把任何两件物体永久性的粘在一起。至尊胶必须被储存在一个内壁涂满了滑溜之油 oil ofslipperiness 的瓶子或罐子中。新发现的这类容器中通常装有1d6+1 盎司的至尊胶。
每盎司至尊胶可以覆盖 1 平方尺的表面，并在 1 分钟后凝固。一旦至尊胶凝固，被粘在一起东西就只能用万溶剂universal solvent、滑溜之油 oil of slipperiness 或祈愿术 wish分开。
法术卷轴 Spell Scroll卷轴，多种稀有度每张法术卷轴都以神秘暗语记录了一道法术。如果该法术属于你的职业法术列表，则你可以阅读该卷轴并施展其法术且无需提供任何材料构材。否则你将无法理解该卷轴。通过阅读卷轴施展的法术同样需要遵循相应法术正常的施法时间。
...
抗法盾 Spellguard Shield护甲（盾牌），极珍稀（需同调）持握此盾牌时，你对抗法术和其他魔法效应所作的豁免检定具有优势，而对你发动的法术攻击简单具有劣势。
湮灭法球 Sphere of Annihilation奇物，传说这个 2 尺直径的黑色球体是多元宇宙中的一个空洞，由一个围绕在它周围的魔法场维持稳定。
...
魅惑法杖 Staff of Charming法杖，珍稀（需诗人、牧师、德鲁伊、术士、邪术师或法师同调）法杖有 10 发充能。持握法杖时，你可以用一个动作并消耗 1 发充能施展魅惑人类 charm person、命令术 command 或通晓语言 comprehend language（使用你自身的法术豁免 DC）。
...
火焰法杖 Staff of Fire法杖，极珍稀（需德鲁伊、术士、邪术师或法师同调）持握这把法杖时，你对火焰伤害具有抗性。
...
冰霜法杖 Staff of Frost法杖，极珍稀（需德鲁伊、术士、邪术师或法师同调）持握这把法杖时，你对冰冻伤害具有抗性。
这把法杖有 10 发充能。持握法杖时，你可以用一个动作并消耗 1 发或更多充能以施展以下法术之一（使用你自身的法术豁免 DC）：
...
治愈法杖 Staff of Healing法杖，珍稀（需诗人、牧师、或德鲁伊同调）这把法杖有 10 发充能。持握法杖时，你可以用一个动作并消耗 1 发或更多充能以施展以下法术之一（使用你自身的法术豁免 DC）：
...
威力法杖 Staff of Power法杖，极珍稀（需术士、邪术师或法师同调）该法杖是一把魔法长棍，你在用它发动的攻击检定和伤害掷骰获得+2 加值。持握法杖时，你的 AC、豁免检定和法术攻击检定获得+2 加值。
...
强袭法杖 Staff of Striking法杖，极珍稀（需同调）该法杖可以用作一把魔法长棍，你在用它发动的攻击检定和伤害掷骰获得+3 加值。
...
虫群法杖 Staff of Swarming Insects法杖，珍稀（需诗人、牧师、德鲁伊、术士、邪术师或法师同调）虫群法杖有 10 发充能，且每天黎明时恢复 1d6+4 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，一群昆虫将吞噬法杖，将其摧毁。
法术 Spells。持握法杖时，你可以用一个动作并消耗若干充能以施展以下法术（使用你自身的法术豁免 DC）：
巨虫术giant insect（4 充能）、疫病虫群 insect plague（5 充能）。
...
蝰蛇法杖 Staff of The Adder法杖，非普通（需牧师、德鲁伊或邪术师同调）你可以用一个附赠动作说出法杖的命令语，并将法杖的头部活化为一条毒蛇，效应持续 1 分钟。你可以用另一个附赠动作再次说出该命令语使法杖还原。
...
贤者法杖 Staff of The Magi法杖，传说（需术士、邪术师或法师同调）该法杖可以用作一把魔法长棍，你用它发动的攻击检定和伤害掷骰获得+3 加值。持握它时，你所发动的法术攻击检定获得+2 加值。
...
秘法锁arcane lock 、侦 测 魔 法 detect magic 、变 巨 /缩 小 术enlarge/reduce、光亮术 light、法师之手 mage hand、防护善恶 pretection from evil and good。
...
巨蟒法杖 Staff of The Python法杖，非普通（需牧师、德鲁伊或邪术师同调）你可以用一个动作说出该法杖的命令语并把它扔到你周边 10 尺范围内的地面上。随后，法杖将变为一条巨蟒 giantconstrictor snake（资料详见《怪物图鉴》）。巨蟒听从你指挥，并以其自身的先攻行动。用一个附赠动作再次说出命令语可将巨蟒变回法杖形态。
...
丛林法杖 Staff of The Woodlands法杖，珍稀（需德鲁伊同调）该法杖可以用作一把魔法长棍被持用，你用它发动的攻击检定和伤害掷骰获得+2 加值。持握它时，你在发动的法术攻击检定获得+2 加值。
...
凋零法杖 Staff of Withering法杖，珍稀（需牧师、德鲁伊或邪术师同调）凋零法杖有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。
...
命令元素石核 Stone of Controlling EarthElementals奇物，珍稀只要这块石头与地面接触，你就可以用一个动作说出它的命令语并召唤一个土元素 earth elemental，如同你施展了召唤元素生物 conjure elemental 法术。直到次日黎明前石头无法再次启动。命令元素石核重 5 磅。
幸运石 Stone of Good Luck (Luckstone)奇物，非普通（需同调）把这块光亮的玛瑙 agate 带在身上时，你在属性检定和豁免获得+1 加值。
阳炎剑 Sun Blade武器（长剑），珍稀（需同调）这件物品看起来像是一把长剑的剑柄。持握剑柄时，你可以用一个附赠动作展开或是解消这把剑完全由光构成的剑刃。
...
应答之剑 Sword of Answering武器（长剑），传说（需阵营与剑相同的生物同调）在灰鹰世界中，已知存在的这种剑仅有九把。这些剑都是传说中被称为“最终遗言 Final Word”的宝剑弗拉格拉克Fragarach 的仿制品。九把剑的名字和阵营各不相同，且把手末端镶有不同宝石。
你在使用此剑发动的攻击检定和伤害掷骰获得+3 加值。
此外，持握该剑时，你可以用反应对你触及范围内任何一个对你造成伤害的生物发动一次攻击。这次攻击的攻击检定具有优势，且这次攻击造成的伤害忽略目标任何伤害免疫或抗性。
窃命剑 Sword of Life Stealing武器（任意剑），珍稀（需同调）当你用这把魔法武器攻击一名非构装非不死的生物并骰出 20 时，目标额外受 10 点黯蚀伤害，且你获得 10 点临时生命值。
锋锐之剑 Sword of Sharpness武器（任意造成挥砍伤害的剑），极珍稀（需同调）用这把魔法剑命中物件时，武器的伤害掷骰将直接取满。
...
复仇之剑 Sword of Vengeance武器（任意剑），非普通（需同调）你在用该魔法武器发动攻击检定和伤害掷骰时获得+1 加值。
...
血光剑 Sword of Wounding武器（任意剑），珍稀（需同调）这把武器造成的伤害只能通过短休或长休恢复。再生、魔法或其他任何手段都无法治愈这种伤害。
...
纯善护符 Talisman of Pure Good奇物，传说（需善良阵营生物同调）这枚护符是至善的象征。接触到护符的非善良非邪恶生物受 6d6 的光耀伤害，邪恶生物则受到 8d6 的光耀伤害。如果这两类生物在自己回合结束时仍握持或携带该护符，则会再次受到等量伤害。
...
法球护符 Talisman of The Sphere奇物，传说（需同调）持握此护符时，你要操控湮灭法球 sphere of annihilation而作的智力（奥秘）检定可以加上双倍熟练加值。另外，如果在你回合开始时你操纵着一个湮灭法球，则你可以用一个动作将法球升起 10 尺外加 10×智力调整值的尺数。
至恶护符 Talisman of Ultimate Evil奇物，传说（需邪恶阵营生物同调）这枚护符是至恶的象征。接触到护符的非善良非邪恶生物受 6d6 的黯蚀伤害，善良生物则受 8d6 的黯蚀伤害。如果这两类生物在自己回合结束时仍握持或携带着护符，则会再次受到等量伤害。
...
触须权杖 Tentacle Rod权杖，珍稀（需同调）这把卓尔制造的权杖属于魔法武器，末端是三条有弹性的触手。持握权杖时，你可以用一个动作令三条触手各对 15尺内的一个目标发动一次攻击。触手的近战攻击加值为+9，命中时造成 1d6 的钝击伤害。如果三条触手都命中了同一个目标，则目标必须进行一次 DC 15 的体质豁免，失败则速度减半，其敏捷豁免将承受劣势，且在 1 分钟内无法执行反应。
另外，在目标的回合内它只能执行一个动作或执行一个附赠动作。目标在自己的每个回合结束时可以重新尝试豁免，成功则终止其相应的效应。
静思卷册 Tome of Clear Thought奇物，极珍稀这本书的文字蕴含魔力，记载了训练记忆力和逻辑思维的方法。如果你在至多 6 日内花费累计 48 小时研究这本书的内容并按其中的方法训练，你的智力和智力上限将提高 2。此后静思卷册将失去魔力，直到一个世纪后才会恢复。
统御卷册 Tome of Leadership And Influence奇物，极珍稀这本书的文字蕴含魔力，记载了影响和吸引他人的方法。
如果你在至多 6 日内花费累计 48 小时研究这本书的内容并按其中的方法训练，你的魅力和魅力上限将提高 2。此后统驭卷册失去魔力，直到一个世纪后才会恢复。
静语卷册 Tome of The Stilled Tongue奇物，传说（需法师同调）这本厚重皮革包覆的卷册封面上钉着一条风干的舌头。
...
通晓卷册 Tome of Understanding奇物，极珍稀这本书的文字蕴含魔力，记载了训练直觉和洞察力的方法。如果你在至多 6 日内花费累计 48 小时研究这本书的内容并按其中的方法训练，你的感知和感知上限将提高 2。此后通晓卷册失去魔力，直到一个世纪后才会恢复。
唤鱼三叉戟 Trident of Fish Command武器（三叉戟），非普通（需同调）该三叉戟是一把魔法武器。它有 3 发充能。持握它时，你可以用一个动作并消耗 1 发充能以对一只有天生游泳速度的野兽施展支配野兽 dominate beast（DC 15）。该三叉戟每天黎明时恢复 1d3 发已消耗的充能。
万溶剂 Universal Solvent奇物，传说这管乳白色的液体散发着浓烈的酒精气味。你可以用一个动作将管中液体倒在触及范围内的某表面上。万溶剂将立刻溶解它接触到的至多 1 立方尺的粘合剂（包括至尊胶sovereign glue）。
恶毒武器 Vicious Weapon武器（任意），珍稀当你用这把魔法武器发动攻击并骰出 20 时，目标额外受到与武器伤害类型相同的 7 点伤害。
斩首剑 Vorpal Sword武器（任意造成挥砍伤害的剑），传说（需同调）你在使用此魔法武器发动的攻击检定和伤害掷骰获得+3加值，并忽略挥砍抗性。
当你命中一名有头生物并投出 20 时，你砍下该生物的一颗头。如果该生物无法在失去头的情况下生存，则它将随之死亡。免疫挥砍伤害、没有头、不需要头、有传奇动作或 DM 认为体型太大无法被斩首的生物免疫斩首效应，但额外受到 6d8的挥砍伤害。
定身魔杖 Wand of Binding魔杖，珍稀（需施法者同调）这把魔杖有 7 发充能，且每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
...
搜敌魔杖 Wand of Enemy Detection魔杖，珍稀（需施法者同调）这把魔杖有 7 发充能。持握此魔杖时，你可以用一个动作说出命令语并消耗 1 发充能。你以此获知 60 尺内离你最近对你有敌意的生物所在方向，但不知道距离，效应持续 1 分钟。魔杖可以感知可见的、隐形的、灵体的、伪装的以及躲藏的生物。你不再持握魔杖时，其效应也随之终止。
该魔杖每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
恐惧魔杖 Wand of Fear魔杖，珍稀（需同调）该魔杖有 7 发充能，且每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
命令 Command。持握此魔杖时，你可以用一个动作并消耗 1 发 充 能 以 命 令 一 名 生 物 逃 跑 或 趴 下 （ 如 同命 令 术command，豁免 DC 15）。
...
火球魔杖 Wand of Fireballs魔杖，珍稀（需施法者同调）这把魔杖有 7 发充能。持握此魔杖时，你可以用一个动作并消耗 1 发或更多充能以施展火球术 fireball（豁免 DC 15）。
施展 3 环火球术 消耗 1 发充能，每多消耗 1 发充能法术环阶也提升一阶。
该魔杖每天黎明时恢复 1d6+1 发已消耗充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
闪电束魔杖 Wand of Lightning Bolts魔杖，珍稀（需施法者同调）这把魔杖有 7 发充能。持握此魔杖时，你可以用一个动作并消耗 1 发或更多充能以施展闪电束 lightning bolt（豁免DC 15）。施展 3 环闪电束 消耗 1 发充能，每多消耗 1 发充能法术环数也增加一阶。
该魔杖每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
搜魔魔杖 Wand of Magic Detection魔杖，非普通这把魔杖有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。持握此魔杖时，你可以用一个动作并消耗 1 发充能以施展侦测魔法 detect magic。
魔法飞弹魔杖 Wand of Magic Missiles魔杖，非普通这把魔杖有 7 发充能，且每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
持握此魔杖时，你可以用一个动作并消耗 1 发或更多充能以施展魔法飞弹 magic missile。施展 1 环魔法飞弹 消耗 1发充能，每多消耗 1 发充能法术环数也增加一阶。
麻痹魔杖 Wand of Paralysis魔杖，珍稀（需施法者同调）这把魔杖有 7 发充能。持握此魔杖时，你可以用一个动作并消耗 1 发充能从魔杖尖端向 60 尺内一名你能看到的生物射出一道纤细的蓝色光线。目标必须成功通过一次 DC 15的体质豁免否则麻痹 1 分钟。目标可以在它的每回合结束时再次尝试豁免，成功则不再麻痹。
魔杖每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
变形魔杖 Wand of Polymorph魔杖，极珍稀（需施法者同调）这把魔杖有 7 发充能，且每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
持握此魔杖时，你可以用一个动作并消耗 1 发充能以施展变形术 polymorph（豁免 DC 15）。
探秘魔杖 Wand of Secrets魔杖，非普通这把魔杖有 3 发充能，且每天黎明时恢复 1d3 发已消耗的充能。持握此魔杖时，你可以用一个动作并消耗 1 发充能以侦测你周围 30 尺内的密门或陷阱。如果范围内有密门或陷阱，魔杖将跳动着指向离你最近的一个。
战法师魔杖 Wand of The War Mage，+1、+2 或+3魔杖，非普通（+1），珍稀（+2），极珍稀（+3）（需施法者同调）持握此魔杖时，你的法术攻击检定获得加值。加值的数值取决于魔杖的稀有度。另外，你在使用法术攻击时忽略半身掩蔽状态。
蛛网魔杖 Wand of Web魔杖，非普通（需施法者同调）这把魔杖有 7 发充能，且每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
持握此魔杖时，你可以用一个动作并消耗 1 发充能以施展蛛网术 web（豁免 DC 15）。
惊异魔杖 Wand of Wonder魔杖，珍稀（需施法者同调）这把魔杖有 7 发充能。持握此魔杖时，你可以用一个动作消耗 1 发充能并选择你周围 120 尺内的一名生物、一个物件或空间中的一点为目标。骰 d100 然后参考下表决定效应。
...
该魔杖每天黎明时恢复 1d6+1 发已消耗的充能。当充能耗尽时，骰一次 d20。如果骰值为 1，魔杖将化为灰烬而毁灭。
武器 Weapon，+1、+2 或+3武器（任意），非普通（+1），珍稀（+2），极珍稀（+3）你在用此魔法武器发动的攻击检定和伤害掷骰获得加值。
加值的数值取决于武器的稀有度。
警戒武器 Weapon of Warning武器（任意），非普通（需同调）这把魔法武器能在危险时给你警告。持有此武器时，你进行先攻检定时具有优势。除非因非魔法睡眠以外的原因陷入失能，否则你和你周围 30 尺内的同伴不会被突袭。如果战斗开始时你或范围内你的同伴处于自然睡眠状态，则该武器的魔法也会将你们唤醒。
诸界之井 Well of Many Worlds奇物，传说这块丝绸般柔软的精致黑色布料折叠起来有手帕大小，展开时则为半径 6 尺的圆形。
...
造风之扇 Wind Fan奇物，非普通持握这把扇子时，你可以用一个动作施展造风术 gust ofwind（豁免 DC 13）。使用扇子后，在次日黎明前每次再次启动它都会累加 20%的失败概率。失败时，扇子也将被撕裂成无魔法的废纸。
飞翼之靴 Winged Boots奇物，非普通（需同调）着装这双靴子时，你获得等同于你步行速度的飞行速度。
你可以使用飞翼之靴连续或间断的飞行 4 小时，并以分钟为单位计算使用时间。如果你在飞行期间耗尽其使用时间，则你将以每轮 30 尺的速度下落，直到着陆。
停止使用靴子达 12 小时将为它恢复 2 小时的使用时间。
飞翼斗篷 Wings of Flying奇物，珍稀（需同调）着装这件斗篷时，你可以用一个动作说出它的命令语，并将斗篷转变为你背上的一对蝙蝠翅膀或鸟类翅膀。这种转变持续 1 小时或直到你再次用一个动作说出命令语。这对翅膀赋予你 60 尺的飞行速度。斗篷使用完毕后，你必须再等待1d12 小时才能再次启动它。
智能魔法物品 Sentient Magic Items一些魔法物品拥有感情和人格。这样的物品可能因为被之前所有者的精神影响，或者是创造时的魔法让他们产生意识，无论如何，这些物品就像角色一样，他们会有怪癖，理想，牵绊或某些缺点。智能魔法物品可能是持有者珍贵的盟友，也可能是他们暗地里不断的烦扰。
...
创作智能魔法物品 Creating SentientMagic Items当你要创造一个智能魔法物品时，你要像创造 NPC 一样赋予物品人格，外加下述内容。
属性值 Abilities智能魔法物品有智力、感知、魅力属性。你可以自己决定物品的属性或者随机分配其属性。如果随机决定的话，为每个属性骰一次 4d6 来随机决定物品的属性。
交流方式 Communication智能魔法物品有一定的交流能力，它们会表达自己的情绪，心灵感应传递思想，或是直接大声的说出来。你可以按照下表决定物品的交流方式感官 Senses随着其智能觉醒。智能魔法物品能够有限的感知它周围的环境。你可以按照下表决定物品感知周围环境的能力。
阵营 Alignment智能魔法物品有阵营。它的创造者或其本性会使其倾向某个阵营。如果没有被赋予，则你可以赋予它一个阵营，或者按照下表决定其阵营。
性格 Characteristics用第 4 章关于设定 NPC 的信息决定智能魔法武器的行为态度、个性特点、理想、牵绊与缺点。你也可以按本章前文“专有特征”一节的信息进行确定。
如果你想随机选择物品的性格，可以尝试忽视或者调整那些不适用于无生机物件的性格。你可以重掷这些性格，直到得到一个你满意的结果为止。
目标 Special Purpose你可以给智能魔法物品设定一个追随的目标，甚至让其排斥除目标外的其他事物。只要持有者的使用方式与物品的目标一致，物品便会愿意和持有者合作。与物品目标违背有可能导致物品与持有者发生分歧，甚至导致物品压制自身的特殊属性。你可以选择一个目标或者按照下表决定目标对抗 Conflict一个智能魔法物品有自己的意愿，并受其性格和阵营影响。如果它的持用者其行为与物品的阵营和目标不符，则物品可能会奋起反抗。这种反抗发生时，该物品需要进行一个检定和持有者的检定进行对抗（该检定基于魅力），如果物品在对抗中获胜，则物品会对持有者有下列一项或多项的要求：
• 物品要求持有者一直携带或着装自己。
• 物品要求持有者丢弃所有它反感的物品。
• 物品要求持有者专心地追求物品的目标。
• 物品要求持有者将它送给别人。
...
智能魔法物品范例 Sample Sentient Items一下描述的一些智能武器都带着悠久的历史。
黑刃 Blackrazor武器（巨剑），传说（需非守序阵营同调）黑刃 藏在白羽山 White Plume Mountain 的地下城中，它像夜空中的繁星一样闪耀，黑色的剑鞘上镶满了切割好的黑曜石。
...
月刃 Moonblade武器（长剑），传说（需中立善良的精灵或半精灵同调）在所有精灵制造的魔法物品当中，最有价值也最受严格保护的就是月刃。在远古时期，几乎每个精灵贵族家族都有这样一柄长剑。几个世纪以来，一些月刃 已经销声匿迹，其魔法也随着家族一起消失。还有一些则伴随其持有者消失在执行伟大任务的路上。因此，只有很少月刃 被保留下来。
...
智能 Sentience。
月刃 是是一把中立善良的智能魔法武器。它的智力是 12，感知 10，魅力 12。它的听觉和黑暗视觉的感官范围为 120 尺。
...
波涛 Wave武器（三叉戟），传说（需海之神的信徒同调）波涛 藏在白羽山的地下城中，这把精致的三叉戟上面雕有波浪、贝壳和海洋生物。尽管你必须是海神的信徒才能与其同调，但波涛 本身也会欣然接受某些新的改变。
...
波涛 一直隐藏着一个关于它天性和目标的秘密。除去它对海神的忠诚，波涛 害怕自己的真正意图是杀死一名海神。
而这一命运或许自己终究无法避免。
征服 Whelm武器（战锤），传说（需矮人同调）征服藏在白羽山的地下城中，它是一把由矮人锻造，极具威力的战锤。
...
神器 Artifacts神器是极之强大且独一无二的魔法物品，而且各自还有着独特的历史与起源。神器可能创自众神或某些能力卓越的凡人，其创作时机往往是历史上的危难之时。彼时，其存在可能对某个王国，乃至整个世界构成威胁，甚至于影响整个多元宇宙。而正是那关键时期，其存在起着至关重要的作用。
...
神器属性 Artifact Properties每件神器都拥有独特的魔法属性，与其他普通魔法物品不同的是，其属性通常强大无比。神器的属性往往同时具有增益和减益的效果。你可以从本节的表格里自主或随机的选取神器的属性，并将这些增减益加入你要使用的神器当中。毕竟每件神器重现世间时，这些属性都会发生改变。
一件神器可以拥有最多四项弱效增益属性以及两项强效增益属性，它同时具有最多四项弱效减益属性以及两项强效减益属性。
破坏神器 Destroying Artifacts神器只会在特殊的情况下遭到破坏，其他情况下神器将无视对其造成的任何伤害。
...
神器范例 Sample Artifacts以下列出的神器曾经出现在 D&D 的世界中。你可以将其作为参考自创新的神器，也可以对其作适当修改后直接使用。
矮人王圣斧 Axe of The Dwarvish Lords武器（战斧），神器（需要同调）曾经有一位年轻的矮人王子，在见证民众面对危困时产生了一个念头：矮人一族需要某种东西促使他们凝聚在一起。
...
圣洁之书 Book of Exalted Deeds奇物，神器（需要由善良阵营生物同调）圣洁之书 是囊括了多元宇宙中所有善良的最终著作，其中凝练的箴言在多个宗教中都有着重要的影响力。圣书不单是为特定信仰贡献的圣经典籍，为其编作的众多作者更在书页之间注入了美德的真实影像，帮助指引众生击退邪恶。
...
秽暗之书 Book of Vile Darkness奇物，神器（需要同调）秽暗之书 是一本肮脏的手抄原稿，其内容包含着难以言喻的恶念，却被受邪恶蛊惑者视为美酒佳肴。凡人本不该知晓其内的秘密，而在这些记载着可怕内容的潦草书页中，简单一瞥所传递的信息就足以引发内心的疯狂。
...
维克那法眼和魔掌 Eye and Hand of Vecna奇物，神器（需要同调）维克那 Vecna 之名即为不语之代表，偶有提及也只能示以静默之音。在其在世的年代，维克那是世上最强大的法师之一。他行驶黑暗魔法之力南征北讨，建立了一个恐怖的大帝国。
...
龙珠 Orb of Dragonkind奇物，神器（需要同调）很久以前，在一个叫克莱恩 Krynn 的世界，精灵与人类对邪恶巨龙发动了一场惨烈的战争。在最后世界接近崩溃之际，大法师诸塔 Towers of High Sorcery 的法师们齐聚一堂，运用其最强大的魔法制作了五颗龙珠 Orb of Dragonkind（或称龙族宝珠 Dragon Orbs），希望以此击败巨龙。制作完成后，五颗龙珠 分给了五座法师塔，手持龙珠 法师们使用神器的力量引诱巨龙们前来，然后用强大的魔法对其进行轰杀。此后，人类联盟凭借龙珠 的力量开始加速通往胜利的步伐。
...
凯斯之剑 Sword of Kas奇物，神器（需要同调）维克那在其力量增长时期曾经任命一名无情的邪恶副官，其名为血手凯斯 Kas the Bloody Handed。除了作维克那的保镖及心腹，这名邪恶反派还担任顾问、军官以及刺客的工作。
...
奥喀斯之杖 Wand of Orcus魔杖，神器（需要同调）阴森恐怖的奥喀斯之杖 总是陪伴在 奥喀斯 Orcus 的身边。它继承了其恶魔领主创造者的邪恶意志与终极目标，想方设法消灭所有生命，然后使物质位面沦为不死的停滞空间。奥喀斯有时会故意让魔杖流落远方，密谋实现某个堕落的目的。
...
其他奖励 Other Rewards冒险家们渴望财富，但有时也会接受其他形式的奖励。本节展示了众神、君主和其他强大存在给予角色的成就，包括一些赋予角色新能力的超自然赠礼；头衔、土地和其他威望加持；
以及一些只有达到 20 级的冒险者才能使用的恩惠。
超自然赠礼 Supernatural Gifts超自然赠礼是由强大魔力存在或原力赋予的特殊奖励。
这类超自然赠礼有两种形式：祝福和护咒。祝福通常由神灵或类神存在所赐。而护咒则通常是某强大精魂，某远古魔法区域，或某个传奇生物的作用。与魔法物品不同，超自然赠礼不是物品，也不需要同调。它可以赋予角色可以单次或多次使用的非凡能力。
祝福 Blessings角色施行某意义重大之事时——即某件能同时引起众神和凡人注意的成就时，便会获得神祇的祝福。杀死横行的豺狼人群，或者在提亚玛特 Tiamat 的高阶祭司意图召唤龙之女王是将其杀死，都会罕见的获得这样的祝福。
一项祝福是对下列成就之一的适当奖励：
...
护咒 Charms护咒是一种获取渠道更为广泛的弱效超自然赠礼。例如，一个法师在某去世大法师的法术书中发现了一个关于魔能的秘密，并利用其作为魔法护咒灌注到自己身上。相似的情况里，解开翔狮兽谜题或从魔法喷泉中饮水的角色，也可以以此获取护咒。传奇生物如远古金龙和独角兽，有时也会给盟友们施加护咒，某些探险家有在发现某个受魔法浸淫而失落久远的地点时，也会受该地影响而获得一份魔法护咒。
...
黑暗视觉护咒 Charm of Darkvision。该护咒可以让得以用一个动作施展法术黑暗视觉 darkvision，且无需任何构材。
使用三次后，该护咒将随即从你身上消失。
...
威望加持 Masks of Prestige有时候冒险者接受的最响亮的奖赏便在获取通行整个国度的威望。各种冒险会为他们带来名誉与权力、盟友和敌人，以及可以传承给后代的头衔。一些领主和女领主曾经也是从平民起步，再随着踏进危险世界中一步步通过勇武行为塑造起自己的名声。
本节详细介绍一些战役冒险中通常能够获得的威望加持。
这些加持通常与宝藏一同获取，但有时也可能会独立出现。
推荐信 Letters of Recommendation黄金供不应求时，冒险者的恩人可能会给他们提供一封推荐信，而非支付钱财。这样的信件通常封在漂亮的开本、盒子或卷轴中以保证安全便携，而且通常都带有书写该信任务的签名和印章。
...
勋章 Medals勋章通常由黄金和其他贵金属精制而成，但其本身意义并不如授勋行为本身的象征意义那样重大。
...
封地 Parcels of Land封地简单直接，通常还会附带一封皇家信件以确定土地作为某种服务的回报而授予。这样的土地通常仍作为当地统治者或统治机构的财产，之算作转租给他人，获赠者必须明白其真正所有人随时可回收土地，特别是当获赠者的忠诚受质疑时更是如此。
...
人情 Special Favors有些奖励会以暂欠人情的方式保留，以让角色们在未来某日需要时索取回报。施予的个体值得信赖时，人情的运作会向好的方向发展。守序善良或守序中立的 NPC 会尽一切努力来履行曾经的承诺，但不会因此违反律法。手续邪恶的 NPC也会履行诺言，但其自身只将其当做一笔交易。中立善良或绝对中立的 NPC 也会为维护自己的名誉而实现承诺。混乱善良的 NPC 更关心为冒险者们做正确的事，他们会履行任何义务而不担心是否要冒个人风险或是否会违反律法。
特权 Special Rights政治当权者可以给角色们授予特权作为奖励，此时通常会以某种官方文件的形式表达。例如，角色们可能被授予的特权包括：在公共场所携带武器，杀死国王的敌人或与公爵的代表进行谈判。角色们可能有权要求获得免费的休息室，聚集社群公布消息，或是有权安排当地的民兵为自己提供协助。
特权只在法律文件规定的情况下才能持续，如果冒险者滥用这些权里，则这些权利也可能会被撤销。
要塞 Strongholds一所要塞通常是给经验丰富的冒险者们授予的奖励，以彰显其精诚效忠某权威政治人物或统治主体，如国王、骑士会或法师议会。要塞的形式多样，它可以是城市中心的加固塔楼，也可以是边境的省区堡垒。虽然要塞可以让角色们按照自己的想法来施行统治，但它所处的地产仍属于国王或当地统治者。如果角色被证实为不忠或配不上该赠礼，那他们也可以被要求或被迫放弃对要塞的监护权。
作为额外的奖励，作为遗赠的要塞可以免除一个月或几个月的维护费用，此后该角色才开始承担其维护职责。见第 6章关于要塞维护的相关信息。
头衔 Titles一位政治上的权威人物有权给他人分配头衔。头衔通常附带有封地（见前文）。例如，授封为“风暴流河伯爵 Earl ofStormriver”或“顿峡湾女伯爵 Countess of Dun Fjord”时还附带有一块封地，其中包括一个同名的聚居地或地区。
角色可以拥有一个以上的头衔，在封建社会中，这些头衔可以传承（或分封）给所有者的孩子。拥有某头衔的角色可能会以适合该头衔的方式行事。依据法令，如果当地统治者或统治主体有理由质疑该角色的忠诚或能力时，其头衔也可以就此被剥夺。
训练 Training角色可以接受特殊训练来代替金钱奖励。这种培训并不常见因而存在一定价值。训练设定有一位经验丰富的训练员（也许是一位退休的冒险者或冠军勇士）愿意出任导师。教练可能是一个为清还女王的人情而前来帮忙的隐居法师或傲慢术士，或是国王卫队的骑士指挥官，强大的德鲁伊结社领袖，住在某偏僻山顶宝塔修道院的古怪武僧，野蛮人酋长，生活在游牧民中作为占卜者的邪术师，或一位所作诗词歌剧家喻户晓人人皆知却时常漫不经心的吟游诗人。
同意接受训练作为奖励的角色在自己的休整期时间里必须与培训师一起度过（参阅第 6 章以获得更多关于休整期活动的相关信息）作为交换，该角色可以确保获得一项特殊增益。
可能的训练收益包括以下内容：
• 该角色在每天的黎明时分获得可持续 1d4+6 日的激励。
• 该角色获得一项技能作为其熟练项。
• 该角色获得一项专长。
传奇恩惠 Epic Boons传奇恩惠，是一种只有 20 级以上的角色才能获得的独特能力。即使达到该等级，角色们也只能随你的意愿，并由你选择合适的时机赋予传奇恩惠。传奇恩惠是角色完成一项重大任务，或完成其他某些可圈可点的事迹后，可获取的绝佳奖励。
...
英勇战斗之恩惠 Boon of Combat Prowess你一次近战武器攻击未命中时，可以选择使之命中。使用该恩惠后，你需要完成一次短休才能再次使用它。
次元旅行之恩惠 Boon of Dimensional Travel作为一个动作，你可以施展法术迷踪步 misty step，且不消耗法术位也不需要任何构材。使用该恩惠后，你需要完成一次短休才能再次使用它。
扭曲命运之恩惠 Boon of Fate你周边 60 尺内除你自身外一个你能看见的生物进行一次属性检定、攻击检定或豁免检定时，你可以骰一次 d10，再将掷骰结果作为加值或减值附加到对方的检定结果中。使用该恩惠后，你需要完成一次短休才能再次使用它。
超凡强韧之恩惠 Boon of Fortitude你的生命值上限提升 40。
高等魔法之恩惠 Boon of High Magic如果你拥有 9 环法术位，则可再获得一个 9 环法术位。
不朽不灭之恩惠 Boon of Immortality你停止衰老。你免疫任何会使你衰老的效应，并且不会因衰老而死。
金身不坏之恩惠 Boon of Invincibility你受到任何来源的伤害时可以将其伤害量减为 0。使用该恩惠后，你需要完成一次短休才能再次使用它。
无敌攻势之恩惠 Boon of Irresistible Offense你可以击穿任何生物的伤害抗性。
操弄幸运之恩惠 Boon of Luck你进行一次属性检定、攻击检定或豁免检定时，可以在该检定结果上增加一次 d10 骰值。使用该恩惠后，你需要完成一次短休才能再次使用它。
魔法抗性之恩惠 Boon of Magic Resistance你为抵抗法术和魔法效应而作的豁免检定具有优势。
无双狙杀之恩惠 Boon of Peerless Aim你可以让自己发动的一次远程攻击检定获得+20 加值。使用该恩惠后，你需要完成一次短休才能再次使用它。
完美健康之恩惠 Boon of Perfect Health你免疫所有毒素和疾病，且你的体质豁免具有优势。
位面旅者之恩惠 Boon of Planar Travel你在获得该恩惠时，先指定一个除物质位面外的存在位面。此后你可以用一个动作施展法术异界传送 plane shift（无需法术位和任何法术构材），法术目标限定为你本身且只能传送到你指定的位面，或者从该位面回到物质位面。使用该恩惠后，你需要完成一次短休后才能再次使用它。
迅捷施法之恩惠 Boon of Quick Casting指定一个你可以施展的 1 至 3 环法术，该法术的施法时间必须是 1 动作。此后你可以用 1 附赠动作施展该法术。
恢复能力之恩惠 Boon of Recovery你可以用一个附赠动作，恢复等于你生命值上限一半的生命值。使用该恩惠后，你需要完成一次短休才能再次使用它。
动能反弹之恩惠 Boon of Resilience你对非魔法武器的钝击、穿刺和挥砍伤害具有抗性。
技能熟练之恩惠 Boon of Skill Proficiency你获得所有技能的熟练项。速度爆发之恩惠 Boon of Speed你的步行速度提升 30 尺。此外，你可以用一个附赠动作执行疾走或撤离动作。使用该恩惠后，你需要完成一次短休才能再次使用它。
法术掌控之恩惠 Boon of Spell Mastery指定一个你可施展的 1 环术士、邪术师或法师法术。此后你可以随意施展该法术的最低环版本且无需消耗法术位。
法术召返之恩惠 Boon of the Recall你可以施展任何你已知或已准备的法术且无需消耗法术位。使用该恩惠后，你需要完成一次短休才能再次使用它。
炽焰魂魄之恩惠 Boon of Fire Soul你 免 疫 火 焰 伤 害 。 你 还 可 以 随 意 施 展 发 生燃 烧 之 手burning hands（豁免 DC 15），且无需消耗法术位也无需使用任何法术构材。
暗夜精魂之恩惠 Boon of the Night Spirit你完全身处微光光照或黑暗环境时，可以用一个动作进入隐形，隐形持续到你执行一个动作或一个反应为止。
暴风降生之恩惠 Boon of Stormborn 你免疫闪电和雷鸣伤害。你可以随意施展法术雷鸣波thunderwave（豁免 DC 15），且无需消耗法术位或使用任何法术构材。
无拘无束之恩惠 Boon of the Unfettered你为抵抗被擒抱而进行的属性检定具有优势。此外，你可以用一个动作直接挣脱擒抱或从任何形式的束缚状态中脱身。
真实视觉之恩惠 Boon of Truesight你获得 60 尺真实视觉感官。
无形无迹之恩惠 Boon of Undetectability你的敏捷（隐匿）检定获得+10 加值，且你无法被预言魔法指定或侦测到，即使是探知的传感器也不行。
传奇恩惠替代品 Alternatives to Epic Boons
也许你会选择给一位 20 级的角色赋予以下某项奖励而不是给其奖励传奇恩惠。这两个选项可被多次赋予同一角色。
属性值加成 Ability Score Improvement。该角色可以自选一项属性提升 2，或选择两项属性各自提升 1。现在你可以将属性提升到 20 以上，上限为 30。
