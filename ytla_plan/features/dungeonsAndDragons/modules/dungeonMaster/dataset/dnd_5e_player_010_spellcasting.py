...
article_spell_description = """
# 第 10 章：施法 Spellcasting
D&D 世界中到处都是魔法，而其最常见的形式就是法术。
本章主要介绍各种法术的施法规则。不同职业的角色以不同的方法学习和准备法术，而怪物们也有其独特的法术用法。但各种不同源头的法术都遵循此处所介绍的规则的规则。
...
# 法术环阶 Spell Level
每项法术都属于 0 至 9 环其中一阶。法术的环阶大致指示了它的强度，低阶（但依然神奇）的法术里有 1 环的魔法飞弹magic missile，而高阶法术里则有惊天动地的 9 环祈愿术 wish。
0 环戏法是一种简便到几乎只靠默念就可以施展的强大法术。
施展高环阶法术时，施法者本身也必须具有较高的施法者等级。
法术的环阶跟角色的等级不是简单的对等关系。展施 9 环法术时，通常需要其角色至少为 17 级，而非 9 级。
...
# 法术位 Spell Slots
不论施法者已知或准备了多少法术，他都只能施展有限的法术，然后就需要靠休息来恢复其施法能力。哪怕施展最简单的法术，操控魔网并引导魔力的过程都会对其身心带来负担，而施展高环阶法术时其消耗则更为严重。因此，每种施法职业的描述都会包含一个列表（邪术师除外），列明角色在特定等级所拥有的每个环阶法术位数量。例如，3 级法师 乌玛拉 Umara具有四枚 1 环法术位和两枚 2 环法术位。
施展某个法术时，施法者消耗一枚与该法术环阶相应，或更高环阶的法术位，相当于用该法术“填上”这个法术位的位置。你可以将法术位想像成一条条特定大小的槽，1 环法术位是小槽，环阶越高对应的槽就越大。1 环法术可以置入到任何大小的法术位中，而 9 环法术则只能置入到一枚 9 环法术位中。
因此，当乌玛拉施展 1 环法术魔法飞弹 时，他消耗四枚 1 环法术位其中之一，并剩下三枚 1 环法术位。
...
当施法者用比法术本身环阶更高的法术位施展法术时，所施展的法术将视为进行升环施法。例如，假设乌玛拉用其 2 环法术位施展法术魔法飞弹，则放出来的魔法飞弹是一个 2 环法术。形象上相当于，该法术自行扩大以匹配施法者为其安排的法术位。
某些法术（诸如魔法飞弹 和疗伤术 cure wounds）在升环施法时会有更强的效应，其细节则会在相应的法术描述中进行说明。

# 着甲施法 Casting in Armor
由于施法时需要专心致志和姿势正确，你在着甲施法时必须具有所着装护甲的熟练项才能成功施展法术。否则你的护甲会阻碍你的动作，并使你分心妨碍施法的进行。

# 戏法 Cantrips
戏法是可以随意施展的法术，它不消耗法术位也不需要预先准备。施法者多次重覆的练习已经将这些法术铭记在其意识里，并灌注了足以重覆施展的魔力。戏法属于 0 环法术。
...
# 附赠动作 Bonus Action
以附赠动作施法的法术特别迅捷。你可以在自已回合内以一个附赠动作施展该法术，前提是你在本回合内必须未执行任何附赠动作。此后在同一个回合里，你只能再施展一道施法时间为 1 动作的戏法，除此之外不能再施展其他法术。

# 反应 Reaction
某些法术可以作为反应进行施展。这类法术通常只需要几分之一秒完成，因此可以用作对事件的应对。如果一项法术可以作为反应进行施放，则法术的详细描述会明确说明你在什么情况下能以它作反应。

# 长时间施展 Longer Casting Times
某些法术（包括进行仪式施法的法术）要数分钟甚至数小时来进行施放。当你所施展法术的施法时间超过一个动作或一个反应时，则你必须在法术施展期间每个自己的回合里都用你的动作来施展该法术，同时还必须保持专注（见下文的“专注”）。
...
大多数法术的施法距离以尺作度量。某些法术只能以你触及范围内的生物作为目标（包括你自身）。还有些法术只影响施术者本身（比如护盾术 shield），这些法术的施法距离即为自身。
以你为源点创造锥状或线状效应的法术其施法距离也算作自身，表示该法术效应源点必须是你。（详见本章的“效应范围”法术一旦施放，其效应的作用范围就不再受施法距离的限制，除非法术另有说明。

# 法术成分 Components
法术的成分是施展该法术时必须满足的物质条件。每项法术的描述中都会说明其施展时是否需要言语 verbal（V）、姿势somatic（S）、材料 material（M）等成分。你只有满足一项法术所有的成分需求后才能施展相应的法术。

# 言语 Verbal（V）
大多数法术都需要咏唱咒文。虽然这些咒文并不给法术供应魔力，但其特定音高共呜的声音组合却可以牵动魔网的弦线来完成施法。因此，如果你被塞住嘴巴或者身处静默的空间（如沉默术 silence 的效应范围），则你将不能施展带言语成分的法术。

# 姿势 Somatic（S）
施法姿势可能包括一记固定的手印或一整套精巧的动作。
如果有法术需要姿势成分，则施法者必须至少空出一只手来完成这些手势。

# 材料 Material（M）
某些法术需要特定的物件才能施展，成分条目里会用括号具体注明。角色们可以使用材料包 component pouch 或法器spellcasting focus（详见第 5 章）取代法术所指定的材料。但如果一项法术的材料成分有指定的价格，则其每次施展时就必须备有这种特殊材料才行。
...
附魔 Enchantment。
附魔法术影响他人的心智，改变或控制他们的行为。这些法术可能令敌人视施术者为朋友，或者强迫生物作出特定行为，甚至像扯线人偶一样操纵其他生物。
塑能 Evocation。
...
用死灵法术创造不死生物（如操纵死尸 animate dead）并不是善良的行径，只有邪恶的施法者者才会常用这些法术。
变化 Transmutation。
...
⚫ 施展另一个需要专注的法术。当你施展另一个需要专注的法术时，你失去原来的专注。也就是说，你不能同时为两个法术进行专注。
⚫ 受到伤害。如果在进行专注时受到伤害，则你必须进行一次体质豁免，豁免成功则保持自己的专注。该豁免 DC 为 10 或所受伤害的一半，以两者中较高值为准。如果你所受伤害存在多个不同的来源，例如同时受到箭矢和龙息攻击，则你必须分别为每个伤害源各投一次豁免。
⚫ 失能或被杀。当你陷入失能或死亡时，也会失去当前的专注。
DM 可以主动判定某些环境可能令你失去专注。例如面对暴风雨中掩面而来的巨浪时，你必须进行一次 DC10 的体质豁免来判断能否维持专注。
...
燃烧之手 burning hands 和寒冰锥 cone of cold 等法术的效应会覆盖一片区域，并同时影响其中的多名生物。
法术的相关描述里会详细说明其效应范围，通常为以下五种形状区域之一：锥状、立方、柱状、线状、球状。效应范围都会有一个爆发魔法能量的源点 point of origin。以下每种形状范围的规则里都详述了如何设置相应的源点。源点通常是一个特定地点，有些法术也会要求用生物或物件作为源点。
...
你的法术的豁免 DC 为 8＋你的法术关键属性调整值＋你的熟练加值＋其他适用的调整值。

# 攻击检定 Attack Rolls
某些法术会要求施法者进行攻击检定来决定法术是否有命中指定的目标。你进行法术攻击的攻击加值等于你的法术关键属性调整值＋你的熟练加值。
大多数需要攻击检定的法术属于远程攻击。其同样要遵守你在敌对生物的 5 尺范围内进行远程攻击时所具有的劣势减益，前提是该生物必须能看见你且其本身不处于失能状态（见第 9章）。
...
凡人无法直接使用这些原生魔法，但却可以借力于一种魔法的织造结构作为连接施法者意识与原生魔法原料的桥梁。被遗忘国度的施法者将其称为魔网 Weave，并奉其本源为女神密斯特拉 Mystra。
施法者们对这种桥梁的命名和认知各不相同，但不管如何称呼，没有魔网，原生魔力就被锁定而无法触及。最强大的大法师在魔网被阻绝的地方也休想用魔法点燃一根蜡烛。而在魔网环绕的地方，施法者就能招来闪电重击敌人，或在一瞬间传送到千里之外，或甚至可以逆转死亡。
所有魔法都与魔网紧密相关，只是不同的法术与魔网的接触方式各不相同。法师、邪术师、术士和诗人的法术称为奥术魔法 arcanemagic。这种法术必须先（通过学习或直觉）理解魔网的工作原理。
