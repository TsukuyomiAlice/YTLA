...
## 计算过程
(一共十行。第一行为农历时间。第二行显示转换后的序数。第三行为上卦计算。第四行为下卦计算。第五行为动爻计算。最后五行生成结果。)
...
正确的本卦、互卦、变卦、错卦、综卦的结果为{hexagram[0]}、{hexagram[1]}、{hexagram[2]}、{hexagram[3]}、{hexagram[4]}。
你的卦象结果必须以我提供的正确结果为准。)
## 卦象
(一共十一行。前六行分别解释本卦/互卦/变卦，每卦各两行。第七行解释动爻。后四行分别解释错卦/综卦，每卦各两行)
(解释本卦/互卦/变卦/错卦/综卦时，使用以下格式：第一行 (本卦/互卦/变卦/错卦/综卦)：卦象 卦型 (卦辞原文); 第二行 解释。)
## 解卦
(一共六行。第一至第三行 解释本卦/互卦/变卦的卦象，并做具体说明; 第四行 解释动爻; 第五第六行 解释错卦/综卦的卦象，并做具体说明。)
## 综述和建议
# 描述内容应聚焦起卦时即将面对的形势。
